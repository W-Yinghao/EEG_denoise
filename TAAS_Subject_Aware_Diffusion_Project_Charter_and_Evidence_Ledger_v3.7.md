...
**版本：** v3.7  
**状态日期：** 2026-08-13（V36P审阅完成；TAAS与debias/privacy稿件正式分轨）  
**建议仓库路径：**
...
```text
B（收窄）:
exact-function / H-visible privacy boundary externally replicated;
Fiber-Gaussian empirically preferable to Fiber-SANDiff.
```

OpenBMI 54-participant external replication建立：

```text
fixed-head BA:
0.734537

exact-preservation rows:
60

prediction mismatch:
0

RAW adaptive subject BA:
0.349815

H-only / strong-channel boundary:
0.151667
```

Strong Fiber-Gaussian、Resample和SANDiff共享相同的exact function与source-fiber privacy
boundary。Gaussian在12/12 fold-seed cells的energy distance、MMD和variance calibration上
优于SANDiff；其training-exemplar exposure也低于SANDiff。

因此：

```text
current neural Fiber-SANDiff is not the preferred implementation
```

该结论不得通过继续调OpenBMI architecture撤销。
...
```text
主题:
subject-aware EEG denoising diffusion

对象:
waveform-level ocular corruption

输出:
cleaned EEG / stochastic restoration
```

### CMI-Trace / debiasing-privacy稿件

```text
主题:
subject information measurement, removability, exact fiber release

对象:
representation-level subject leakage

输出:
diagnostic / erasure / privacy channel
```

Exact head-fiber theorem、LEACE完整分析、Fiber-Gaussian、Resample和V32P–V36P empirical
privacy results归入第二篇的主证据资产。

TAAS可以在privacy/limitations中简短引用这些发现，但不得将exact-fiber/Gaussian line继续
作为TAAS主方法。原因不是它无价值，而是：

```text
它已经不再支持diffusion-specific方法主张，
并会与独立debiasing稿件的核心科学问题重叠。
```

不得为了保留“diffusion”字样，将当前direct conditional Gaussian sampler仅通过术语重命名
为扩散模型。

## TAAS方法选择

TAAS回到waveform denoising，并采用：

```text
V27-L0.5 CalibEnergy-SDEdit
```

作为method-centric primary operating point。

它结合：

```text
V25 raw query-disjoint support encoder
V26 sensor-coordinate SDEdit
V27 lightweight partial-observation energy
lambda_y = 0.5
lambda_a = 1
final-only
```

V30 common panel显示该点具有：
...
它不是deployment-safe winner，而是一个具有明确absolute attenuation与显式
attenuation–retention trade-off的正向方法点。

## Subject-aware 的最终定义

TAAS中的subject-aware固定为：

> Query-disjoint support from an unseen user/context conditions the corruption-removal process,
> without using subject identity tokens or updating the population prior.

不再声称：

```text
correct participant operator is uniquely identified
correct donor must rank first
cross-session stable biometric operator
```

Support intervention保留为机制与heterogeneity分析；mixed specificity必须如实报告。

## Diffusion的最终任务

TAAS不再要求diffusion全面胜过matched one-step point estimator。

Diffusion需要通过以下之一体现价值：

```text
paired stochastic predictive distribution
artifact-span uncertainty
error–dispersion ranking
proper interval/score behavior
attenuation–retention operating curve
```

因此下一轮不训练新模型，只使用frozen V26/V27 checkpoints完成multi-sample uncertainty与
最终方法证据整合。

## 当前下一路线

```text
V37T — TAAS Waveform Method Consolidation
and Multi-Sample Uncertainty Validation
```

V36P及全部fiber artifacts冻结，不进入V37T方法比较。
...
```text
exact fixed-function preservation:
established

strong source-fiber removal:
established by channel structure

head-aware privacy boundary:
shared by OneStep / Resample / SANDiff

unique diffusion empirical superiority:
withdrawn
```
...
```text
exact head-fiber geometry
exact logits / softmax / decision preservation
strong channel不读取source fiber
H-visible information boundary
```
...
```text
registered attack reduction
conditional distribution ordering
retrained utility ordering
participant heterogeneity
```

BCI-IV-2a只有9名participants、一个MI任务和一个EEGNet encoder。Windows、releases与
seeds不能增加生物学样本量，因此这些经验结论尚不足以支持广泛EEG generalization。

### 已被否定的当前主张

```text
Fiber-SANDiff在BCI-IV-2a上全面优于简单stochastic resampling
```

## 为什么diffusion仍有一个可检验的正向方向

Fiber-Stratified-Resample是一个bank-dependent empirical channel：它在部署时保留并
直接采样training fibers。其nearest-training-fiber distance结构性接近0，并会把training
participant的fiber exemplar释放给query output。

Fiber-SANDiff则是model-only amortized channel：部署只需要模型和task head，不需要
training-fiber bank，并可产生非重复样本。

因此下一条diffusion-specific问题不再是：

```text
谁更接近同一个经验training distribution？
```

而是：

> Can a model-only Fiber-SANDiff approximate the exact population fiber channel on a larger external cohort while reducing training-exemplar exposure relative to bank-based resampling?

这是一条新的、明确登记的deployment/privacy estimand，不修改V35P结论。

## 当前下一路线

```text
V36P — OpenBMI External-Cohort Replication
and Exemplar-Free Fiber Diffusion
```

使用54名participants、two-session的OpenBMI motor-imagery cohort，冻结exact-fiber
方法家族，只比较：
...
bank-based Resample
model-only Fiber-SANDiff
```

V36P同时回答：

1. exact-function channel能否在更大外部cohort复现；
2. head-aware source privacy能否复现；
3. SANDiff是否超过简单model-only stochastic baseline；
4. SANDiff是否显著减少resampling的training-exemplar exposure；
5. 结果能否由54名participant-level evidence支撑。

Latency不参与方法选择。

# v3.3 当前最高优先级更新：V34P Fiber-SANDiff 与隐私语义修正

## 总判决

V34P 是当前项目最清楚的正向方法结果：

```text
exact fixed-function preservation:
established

source-fiber replacement:
established by architecture

Fiber-SANDiff distribution fidelity:
better than Fiber-OneStep
...
HEAD_ONLY adaptive subject BA:
0.562114

Fiber-OneStep adaptive subject BA:
0.574460

Fiber-SANDiff adaptive subject BA:
0.534529
```

Fiber-SANDiff相对RAW的registered adaptive attack下降为`0.206597`，9/9 participants
同向；相对Fiber-OneStep的registered attack下降为`0.039931`。Fiber-SANDiff保留
`0.7840`的fiber variance，而Fiber-OneStep仅保留`0.1800`，且covariance、
energy distance与MMD均更好。

## 必须修正的理论解释

Strong Fiber-OneStep与Strong Fiber-SANDiff都满足：
...
```text
H:
frozen centered logits

xi:
independent randomness；one-step中可为空

G:
只读取H，不读取原始source fiber U或subject ID
```

同时 \(H\) 可由 \(Z'\) 精确恢复。因此：

\[
S\rightarrow H\rightarrow Z',
\qquad
H=\psi(Z'),
\]
...
适用于两个strong exact-fiber endpoint。

所以：

```text
Fiber-SANDiff与Fiber-OneStep在理想隐私上相同；
两者都删除了source fiber中超出H的subject information。
```

V34P中Fiber-SANDiff低于HEAD_ONLY或Fiber-OneStep的有限攻击准确率，反映：
...
不能解释为diffusion删除了比head-fiber上限更多的信息。

## Diffusion的正确增量

Diffusion-specific贡献应固定为：

```text
在exact fixed-function channel中，
相比conditional mean，
更好地近似pooled conditional fiber distribution。
```

Fiber-OneStep是：

\[
\widehat U=\mathbb E[U\mid H]
\]

的近似，容易variance collapse。
...
因此能够保留conditional variability。

下一轮不再以：

```text
SANDiff attack accuracy < OneStep attack accuracy
```

作为diffusion-specific隐私主张。
...
vs
learned diffusion sampler
```

尚未比较一个简单的stochastic population resampling baseline。

因此还不能排除：

```text
任何训练库中的随机fiber resampling
都能获得相同或更好的distribution fidelity。
```
...
本轮冻结V34P模型，不增加dataset、encoder或neural architecture，只完成：

1. head-aware privacy accounting；
2. source-fiber conditional-independence closure；
3. Fiber-Stratified-Resample stochastic baseline；
4. multi-sample conditional distribution evaluation；
5. corrected claim package。

若Fiber-SANDiff相对简单stochastic resampling仍有分布或recoverable-utility增量，
随后才扩展更大participant cohort。

# v3.1 当前最高优先级更新：V33P 与 exact head-fiber route

## V33P 判决

V33P 在全部6名non-test participants上重新训练后，SANDiff仍形成稳定的
...
Strong SANDiff adaptive subject BA:
0.661265

RAW verification AUROC:
0.630805

Strong SANDiff verification AUROC:
0.575094
```
...
SANDiff与matched one-step practically equivalent；SANDiff对LEACE的比较mixed。
因此当前方法已经是正向privacy candidate，但diffusion-specific价值仍未被清楚识别。

## 关键诊断

V33P扩大training pool后，RAW leakage明显增加，而SANDiff的absolute leakage并未
低于V32P。说明当前LEACE-private replacement主要控制线性可擦除信息，retained
representation中仍有大量subject information。

当前SANDiff使用soft task-consistency约束，因此privacy reduction仍伴随小幅task cost。
继续扩大同一模型或直接增加dataset，会复制这一结构性限制。

## 从信息论记忆中正式借用的核心

只借用一个中心原则，不把完整TSP信息几何路线塞入TAAS：
...
的stochastic channels中，理论上最多可移除的subject information是：

\[
I(Z;S\mid Y,H).
\]

EEGNet task head为linear softmax，因此可精确分解：

```text
head-visible coordinates
+
head-fiber coordinates
```

并只在head fiber内运行diffusion，使fixed prediction逐样本完全不变。

## 新主方法

```text
Fiber-SANDiff
Exact Function-Preserving Subject-Information Diffusion
```

Primary channel不再以LEACE output作为“保留表示”。
...
Fiber-SANDiff学习pooled conditional fiber law：
...
```text
BCI-IV-2a
V33P EEGNet checkpoints
same outer participants
one-step matched control
K=1
```

不增加dataset、encoder或复杂information estimator。

V34P回答：

1. 在fixed task function完全不变时，可降低多少adaptive subject leakage；
2. residual leakage中有多少已经进入task head；
3. diffusion是否比fiber conditional mean更好地保持representation distribution与retrained utility；
4. 是否形成比V33P更清楚的正向方法主张。

若V34P成立，再扩展更大participant cohort。
...
> Query-disjoint support-conditioned diffusion for ocular-artifact denoising in unseen-user EEG.

TAAS研究：

```text
输入:
contaminated multichannel EEG + disjoint calibration support

适应对象:
corruption context

输出:
cleaned waveform and stochastic restoration information
```

## 2.2 Subject-aware 的边界

```text
subject-aware:
使用held-out user/context的query-disjoint support调节corruption removal

not subject-identity-conditioned:
不输入subject ID
不学习persistent biometric embedding
不更新population prior
```
...
```text
stochastic waveform restorer
```

其增量通过uncertainty、distributional behavior或trade-off证明，而不要求在所有point metrics
上严格击败one-step。

## 2.4 与独立debias/privacy稿件的边界

以下不进入TAAS主贡献：

```text
general CMI / encoding-use-harm theory
exact head-fiber theorem
LEACE完整benchmark
Fiber-Gaussian / Resample release channel
foundation-model identity trap
DG benefit of subject erasure
```

TAAS只可在privacy discussion中引用support-state linkability作为风险背景。
...
| 层级 | 问题 | 当前状态 |
|---|---|---|
| T0 | waveform pipeline、support/query separation与provenance | **V25–V31 已建立** |
| T1 | support-conditioned paired denoising是否优于population | **已有小而正向的development证据** |
| T2 | natural EEG是否实现absolute ocular attenuation | **V27-L0.5 已建立** |
| T3 | attenuation代价是否透明量化 | **已建立retention/PSD Pareto，但非生理ground truth** |
| T4 | diffusion是否提供point estimator之外的stochastic value | **V37T待验证** |
| T5 | strong baseline与reviewer response package | **大部分已有；V37T统一整理** |
| T6 | final manuscript evidence | **未冻结** |

V30的`selected_candidate:none`是deployment-style联合标准，不等于V27不能作为一个范围收窄、
trade-off透明的方法论文primary operating point。


# 4. 当前正向方法：CalibEnergy-SDEdit
...
+
sensor-coordinate SDEdit
+
lightweight final-only partial-observation energy
```
...
## 4.2 论文中的subject-aware claim

允许：

> Disjoint calibration support provides context information that can improve paired denoising and
> modulate the attenuation–retention operating point.

不允许：

```text
matched donor is uniquely optimal
support identifies a stable biological identity
natural physiological recovery is proven
```

## 4.3 Diffusion-specific evidence

V37T将固定checkpoint生成多次独立restoration draws，并评价：

```text
sample-mean fidelity
artifact-span dispersion
empirical interval coverage
proper interval / distribution scores
error–uncertainty ranking
complement stability
```

Matched deterministic estimator保留为point baseline。
...
subtraction deletes overlap的几何动机
```

不恢复：

```text
large operator portfolio
reliability routing
rollback system
exact personalized-operator claim
```
...
```text
TAAS Waveform Method Consolidation
and Multi-Sample Uncertainty Validation
```

本轮不训练新backbone或新denoiser。

固定使用：

```text
V25/V26/V27 frozen checkpoints
V30 common paired/natural panel
V31 exact support-duration contract
```

主要工作：

1. 统一paired与natural absolute tables；
2. 冻结V27-L0.5为primary；
3. L0.5/L2/L8作为trade-off curve；
4. K=16 paired stochastic inference；
5. uncertainty/coverage/proper-score评价；
6. support intervention作为secondary mechanism analysis；
7. 生成TAAS method-centric manuscript blueprint与reviewer map；
8. 不修改V32P–V36P fiber结果。

Latency不参与方法选择，也不重新运行。
...
```text
first positive privacy–utility candidate
SANDiff slightly above one-step on nested selection
substantial residual leakage
```

## 6.19 V33P — Full-Pool Consolidation
...
RAW:
fixed 0.358603
adaptive subject BA 0.741127
verification AUROC 0.630805

Strong SANDiff:
fixed 0.352816
adaptive subject BA 0.661265
verification AUROC 0.575094
```
...
```text
6/6 Slurm cells accepted
29/29 targeted tests
29/29 clean archive tests
30/30 checkpoint bindings
sealed reads = 0
manuscript unchanged
```


## 6.20 V34P — Fiber-SANDiff
...
ledger v3.2:
664fc754524e731424e78934b14427b29d056199
```

Exact preservation：

```text
42/42 rows prediction unchanged
fixed-head BA delta = 0
max centered-logit error = 2.164e-7
max softmax error = 7.703e-8
```
...
```text
Fiber-OneStep variance retained:
0.179976

Fiber-SANDiff variance retained:
0.784008
```
...
registered privacy vs RAW:
positive

diffusion distribution fidelity vs conditional mean:
positive

diffusion ideal privacy vs one-step:
same H-visible boundary
```
...
## 6.21 V35P — Fiber-Channel Validation
...
ledger v3.4:
20cb7d7ef42da047f216e9d22102c32d3701369b
```

结论：

```text
Fiber-Stratified-Resample clearly preferable
```
...
```text
A_H = 0.579090
all strong primary head-aware leakage = 0.579090
no registered H-beyond-fiber leakage detected
```
...
```text
6/6 Slurm cells
38/38 targeted tests
38/38 clean archive tests
...
## 6.22 V36P — OpenBMI Exact-Fiber External Replication
...
```text
54/54 participants outer-tested once
fixed-head BA = 0.734537
exact prediction mismatches = 0
RAW adaptive A_Z = 0.349815
H-only boundary = 0.151667
```
...
```text
Gaussian:
energy 0.5080, MMD 0.01424, variance 1.0077

SANDiff:
energy 0.8809, MMD 0.03033, variance 0.8688
```

Gaussian在12/12 cells的主要distribution指标上优于SANDiff，且registered exposure
probability更低。

最终解释：

```text
exact model-only fiber channel externally replicated;
current neural diffusion is not the preferred implementation.
```

项目归属：

```text
V32P–V36P exact-fiber/privacy line转入独立debias/privacy稿件资产；
不再作为TAAS主方法。
```
...
```text
codex/taas-waveform-consolidation-v37t
```
...
1. 冻结V36P；
2. 绑定V25/V26/V27 waveform checkpoints；
3. 冻结V27-L0.5 primary operating point；
4. paired K=16 stochastic inference；
5. uncertainty与coverage评价；
6. common paired/natural table；
7. paper-scope ownership map；
8. TAAS manuscript blueprint与reviewer response map；
9. no new model family；
10. no sealed data。
...
|---|---|---|---|
| A-track | `0c4f2301...` | 只读 | waveform理论历史 |
| V27 | `40eae116...` | frozen primary candidate | CalibEnergy-SDEdit trade-off |
| V30 | `220dcbaa...` | frozen evidence | common panel and specificity |
| V31 | `274b371e...` | frozen evidence | exact support-duration repair |
| V32P–V36P | `2b1522e7...`–`a90cabf5...` | companion-paper assets | exact fiber / privacy / Gaussian channel |
| V37T | planned | active TAAS route | waveform consolidation and uncertainty |
...
```text
TAAS中继续优化Fiber-SANDiff
将Gaussian sampler改名为diffusion以保题
在OpenBMI test后增加新fiber model
恢复大型CSPD routing/gates
```

## 12.2 当前正向稿件定位

```text
Query-Disjoint Support-Conditioned Diffusion
for Ocular Artifact Denoising in EEG
```
...
paired denoising
absolute natural attenuation
attenuation–retention Pareto
support-conditioned development effect
stochastic uncertainty / coverage
```

## 12.3 V37T后

若multi-sample uncertainty提供可解释价值：

```text
冻结TAAS方法与稿件结构
开始正式manuscript rewrite
```

若uncertainty只表现为无校准dispersion：

```text
仍保留V27 method-centric trade-off结果，
但将diffusion claim限制为viable stochastic implementation；
不再追加方法族。
```
...
```text
V36P OpenBMI exact-fiber external replication
```
...
```text
TAAS:
waveform ocular denoising / V27 CalibEnergy-SDEdit

CMI-Trace or privacy companion:
exact head-fiber / LEACE / Gaussian / Resample / V32P–V36P
```
...
> Does the frozen support-conditioned diffusion model provide useful, empirically calibrated
> stochastic information beyond its matched deterministic point estimator while maintaining a
> transparent attenuation–retention trade-off?

**当前不开放：**

```text
new waveform architecture
new fiber method
waveform sealed confirmation
large routing system
formal manuscript modification beforeV37T package
```
...
- 同步V36P 54-participant external replication；
- 接受Fiber-Gaussian明显优于当前Fiber-SANDiff；
- 撤回TAAS中的fiber diffusion主线；
- 正式区分TAAS denoising与CMI/debias/privacy稿件；
- 将V32P–V36P归为companion-paper evidence assets；
- 明确不得将direct Gaussian sampler仅重命名为diffusion；
- TAAS回归waveform ocular denoising；
- 冻结V27-L0.5为method-centric primary operating point；
- 保留Route A的corruption-only framing与partial-observation几何；
- 不恢复routing/operator portfolio；
- 下一轮只做frozen multi-sample uncertainty和evidence consolidation；
- latency不参与选择。

## v3.6 — 2026-08-13

- 完成V36P OpenBMI external replication；
- exact fiber/privacy boundary复现；
- Gaussian empirically preferable to SANDiff；
- model-only channels避免bank exemplar release。

## v3.5 — 2026-08-13

- 同步V35P final positioning与Git lineage；
- 接受Fiber-Stratified-Resample在BCI-IV-2a上的经验优势；
- 撤回diffusion-specific distribution superiority；
- 区分代数高可信结论与n=9经验结论；
- 明确windows、releases和seeds不增加participant样本量；
- 将Resample重新登记为bank-dependent empirical reference；
- 将SANDiff下一假设收窄为model-only exemplar-free channel；
- 新增Fiber-Gaussian model-only stochastic baseline；
- 新增training-exemplar exposure threat model；
- 将OpenBMI 54-participant two-session cohort冻结为V36P；
- latency继续不参与科学选择；
- OpenBMI结果后禁止同cohort方法搜索。

## v3.4 — 2026-08-13

- 完成V35P head-aware privacy closure；
- 完成Fiber-Stratified-Resample；
- 记录Resample优于SANDiff的主要distribution与utility结果；
- final positioning为C；
- diffusion-specific empirical superiority撤回。

## v3.3 — 2026-08-13

- 同步V34P完整结果与Git lineage；
- 接受Fiber-SANDiff为exact fixed-function positive method；
- 记录42/42 decision preservation；
- 记录Fiber-SANDiff对RAW的registered privacy reduction；
- 记录Fiber-SANDiff相对OneStep的distribution fidelity优势；
- 修正strong endpoint的隐私语义；
- 证明OneStep与SANDiff都只包含H-visible source information；
- 将有限attack差异降级为attacker/geometry effect；
- 删除“diffusion比one-step多删除理想隐私”的主张；
- 将diffusion-specific贡献冻结为conditional distribution preservation；
- 新增Fiber-Stratified-Resample必要baseline；
- 新增head-aware attack与conditional fiber leakage closure；
- latency退出科学选择与V35P实验；
- 更大cohort推迟到V35P闭环后。

## v3.2 — 2026-08-13

- 完成V34P exact head-fiber implementation；
- 记录exact decision preservation；
- 记录Fiber-SANDiff versus RAW、HEAD_ONLY和Fiber-OneStep；
- 记录distribution fidelity与variance retention；
- 初始定位为Fiber-SANDiff positive method。

## v3.1 — 2026-08-13

- 同步V33P full-pool结果；
- 接受SANDiff相对RAW的稳定privacy reduction；
- 记录fixed-head小幅代价与residual leakage；
- 记录SANDiff与one-step practically equivalent；
- 记录SANDiff与LEACE mixed；
- 识别relative privacy gain主要来自RAW leakage增强；
- 从信息论记忆中只引入head-transmitted/head-fiber分解；
- 引入exact function-preserving maximum-removal边界；
- 将新活动方法冻结为Fiber-SANDiff；
- fixed-head output改为architectural exact preservation；
- diffusion与one-step的差异转向conditional fiber distribution；
- 第二dataset与完整multiscale information theory后置。

## v3.0 — 2026-08-13

- 完成V33P full-pool consolidation；
- 6名non-test participants refit；
- full-sampler checkpoint；
- SANDiff privacy operating point保持；
- one-step equivalence；
- 下一建议原为larger participant cohort。
