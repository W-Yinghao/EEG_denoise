...
The held-out single pass reproduces the no-harm conclusion on both tasks (SSVEP $-0.002$, interval $-0.012$ to $+0.007$; ERP AUC $-0.002$, interval $-0.009$ to $+0.010$; ERP preservation 0.966), with the guided-versus-unguided direction preserved but not adjudicable at $n=8$.

Two extensions test whether this conclusion is an artifact of decoder capacity or of the panel. First, replacing the shallow decoders with EEGNet-8,2 (within-participant five-fold cross-validation, identical folds for every arm, fresh initialization per arm) leaves every diffusion arm within noise of raw on both cohorts, while \emph{revealing} harm in the classical arms that the linear decoder had missed: on the development ERP task the deep decoder places ICA at 0.745 and the eye-subspace projection at 0.740 against raw 0.769, with the calibrated arms at 0.765--0.774. Deep capacity finds classical-arm damage, not diffusion-arm damage. (On three-class SSVEP the deep decoder itself is underpowered at these trial counts, 0.34--0.37 everywhere, and adjudicates nothing; the CCA endpoint remains the informative one.) Second, the same no-harm result reproduces on a third panel with recorded EOG channels: on BCI Competition IV-2a (nine participants, four-class motor imagery, official train/evaluation split, EEGNet decoding), both the linear-operator route ($-0.008$, interval $-0.027$ to $+0.008$) and the full calibrated-diffusion route ($-0.011$, interval $-0.043$ to $+0.022$) land within noise of raw, as do the population and no-calibration ablations. Across three panels, three decoders, and both delivery routes, calibrated cleaning neither purchases nor costs decoding accuracy; its value lives in the waveform, interval, and retention currencies. It also sharpens the point that decoding accuracy, waveform preservation, and retention are different currencies: on held-out users ASR's ERP AUC is significantly \emph{above} raw ($+0.042$, interval $+0.011$ to $+0.075$) while its ERP preservation collapses to 0.637 --- aggressive variance removal can help a linear classifier while destroying the morphology it decodes from --- and the eye-subspace projection significantly harms SSVEP decoding ($-0.010$, interval $-0.013$ to $-0.006$). The calibrated system is the one method in the comparison that stays healthy in all three currencies at once, which is the claim a deployable artifact-removal loop actually needs.
...
A sealed confirmation cohort settles both questions at scale. Fifty-five EEGEyeNet participants never touched during development were opened once, against a pre-registered protocol with frozen settings and a both-verdicts-reported disclosure rule. The conditioning endpoint confirmed: matched-minus-zero gain $+0.081$ (interval $+0.058$ to $+0.109$; 53 of 55 participants positive), matched-minus-mismatched $+0.166$ (55 of 55), flat from 30 to 259 training participants, zero guard exclusions --- the development effect reproduced on a second corpus at four times the cohort. The interval endpoint transferred with one honest miss: the development-frozen temperatures produced 0.830/0.891 coverage at nominal 0.80/0.90 on the new corpus (within the pre-registered tolerance), but the CRPS ordering between the propagation-augmented and temperature-only widths did \emph{not} reproduce (0.1375 versus 0.1363), so the physically-motivated-width advantage of Section~\ref{sec:res-uq} is a property of the ambulatory corpus that we do not claim beyond it; per-participant 80\% coverage on the sealed cohort spans 0.22--1.00 and is disclosed in full.
...
\subsection{Context against supervised and diffusion denoising benchmarks}
\label{sec:res-benchmarks}

Three reproduction studies place the system against the published landscape, run under a preserve-and-report discipline: upstream code is imported at a pinned commit, upstream defects are kept and disclosed rather than silently repaired, and every protocol choice we were forced to make is frozen before numbers are seen.

\textbf{Single-channel benchmarks are not this system's habitat, and we report that plainly.} On the EEGdenoiseNet bench re-run at the 11-level $-5$ to $+5$\,dB grid used by EEGDfus, our noisy-conditioned diffusion denoiser (CC 0.888 EOG / 0.763 EMG) beats the unconditional prior and two of five supervised baselines but loses to SimpleCNN (0.934) and NovelCNN --- and nothing we trained approaches the published EEGDfus figures. Transplanting the same single-channel model onto the semi-simulated SSED corpus is an honest negative: trained on SSED, zero-shot, or fine-tuned, it lands below the identity baseline (CC 0.48--0.49 versus 0.70), because full-generation sampling injects variance into rows that are largely clean. The subject-calibrated multichannel loop, not the single-channel transplant, is where the method's value lives.

\textbf{Reproducing EEGDfus on SSED quantifies a split defect.} Their released SSED pipeline trains successfully under our seeds, and under their released data split we obtain CC 0.980, consistent with their published 0.992. Their split code, however, draws the test indices from the full index range rather than the held-out pool, so released-split test rows overlap training rows; repairing the split drops performance to CC 0.889 (RRMSE$_t$ 0.085 to 0.373). We report both protocols side by side. A phase-locking analysis agrees: the contamination is low-frequency, identity PLV is already near 1.0 above the alpha band, EEGDfus repairs delta-band PLV (0.68 to 0.87 under the strict split), and our transplant degrades it --- consistent with the reconstruction metrics.

\textbf{The DS-DDPM release trains but does not support its published evaluation.} The subject-separation model of our collaborators trains to convergence under their loop mirrored verbatim. But the release contains no code path that denoises recorded EEG --- both samplers start from Gaussian noise --- so the paper's separation inference had to be reconstructed from their own primitives (disclosed in full); the classifier behind their Table I is likewise unreleased, and across ten frozen protocol variants (three data arms, four documented recipes) their backbone never leaves chance level on the four-class task (25--28\% against a published 50.6/52.9), while a standard EEGNet-8,2 reaches 66\% within-participant on the same trials. Finally, the generated subject noise shows no subject specificity under a frozen spectral-signature metric (matched 0.392 versus mismatched 0.393). We do not conclude that the published numbers are wrong; we conclude that they are not reproducible from the release, which is the reproducibility statement a benchmark comparison is obliged to make.
