...
| 层级 | 问题 | 当前状态 |
|---|---|---|
| P0 | 数据、participant/session split和adaptive attack协议是否有效 | **V32P/V33P 已建立** |
| P1 | RAW representation是否存在cross-session subject leakage | **已建立** |
| P2 | SANDiff是否形成privacy–utility candidate | **V33P 已建立** |
| P3 | 当前LEACE-private replacement是否优于one-step/LEACE | **未建立；practically equivalent / mixed** |
| P4 | fixed task output完全不变时，可移除多少subject leakage | **V34P 待运行** |
| P5 | diffusion是否比fiber one-step更好保持conditional representation law | **V34P 待运行** |
| P6 | 更大participant cohort是否复现 | **后续轮次** |
| P7 | 是否形成最终TAAS修回证据 | **未建立** |

V33P证明：

```text
privacy reduction可以稳定发生
```

但没有证明：

```text
当前LEACE decomposition最合适
或
iterative diffusion明显优于one-step
```

V34P将task utility从soft empirical objective改成exact architectural constraint。


# 4. 当前正向方法：Fiber-SANDiff

## 4.1 Fixed-function decomposition
...
以及冻结linear task head。用centered logits定义：

\[
H=CZ.
\]

将representation正交分解为：
...
任何逐样本保持task output的channel都无法删除已经进入 \(H\) 的subject dependence。

因此，V34P的目标不是把subject attack降到chance，而是：

```text
将full-representation leakage推向H-only leakage floor，
同时保持fixed task output数值不变。
```

## 4.3 Fiber replacement

训练一个population fiber model：

\[
U'\sim p_\theta(U\mid H).
\]

Primary inference不使用：
...
## 4.4 Matched deterministic control
...
它代表pooled conditional mean replacement。
...
## 4.5 Diffusion-specific价值

Point privacy与fixed-head utility可能仍与one-step接近。

Diffusion的预期附加价值是：

```text
保留conditional fiber variability
减少mean-collapse
改善retrained utility
更接近pooled class/logit-conditional representation distribution
提供连续stochastic privacy channel
```

若这些均未出现，one-step仍是合理结论，不继续扩展模型动物园。
...
```text
Fiber-SANDiff
Exact Function-Preserving Privacy Channel
```
...
V33P full-pool EEGNet checkpoints
same 3 outer test groups
2 seeds
K=1
10 reverse steps
```
...
1. centered logits与softmax是否逐样本精确保持；
2. H-only leakage是多少；
3. Fiber-SANDiff能否降低full representation leakage；
4. fixed-head BA/ECE是否严格不变；
5. retrained-head utility是否保持；
6. Fiber-SANDiff是否比Fiber-OneStep更好保持fiber distribution；
7. latency是否可接受。
...
第二dataset
foundation encoder
完整CMI/score-operator estimation
nonlinear manifold fiber
membership inference
support-specific channel
new waveform experiment
```

信息论记忆中的multiscale Fisher operator、CMI estimator和amortized oracle仍属于独立TSP路线，不进入V34P主实验。
...
1. 冻结V33P EEGNet/head；
2. exact centered-logit fiber decomposition；
3. H-only leakage floor；
4. Fiber-OneStep；
5. Fiber-SANDiff；
6. exact prediction-preservation tests；
7. adaptive privacy与retrained utility；
8. conditional fiber distribution diagnostics；
9. 不增加dataset/encoder；
10. 结果后升级ledger v3.2。
...
| V33P | `5292c1a5...` | latest complete | full-pool privacy candidate; one-step equivalence |
| V34P | planned | active | exact head-fiber SANDiff |
...
这不是重新开一个方法动物园，而是用一个exact task-preserving geometry修复V33P的核心限制。

当前不应直接扩大到第二dataset，因为：

```text
V33P的absolute sanitized leakage没有继续下降；
当前task preservation只是soft loss；
one-step与diffusion的point result等价。
```

V34P应先确认：

```text
能否在fixed decision完全不变时，
删除head-fiber中的subject information，
并由diffusion保留one-step会压缩的conditional variation。
```
...
```text
Fiber-SANDiff正向:
扩展更大participant cohort

Fiber-OneStep等价或更好:
保留exact fiber方法，
将diffusion定位为可选stochastic implementation

两者均无privacy reduction:
停止BCI-IV-2a方法搜索，
转入更大cohort验证head-transmitted leakage是否占主导
```
...
```text
V33P SANDiff Full-Pool Consolidation
```
...
```text
1. SANDiff相对RAW显著降低cross-session subject leakage。
2. fixed-head task utility只有小幅、异质代价。
3. SANDiff与matched one-step practically equivalent。
4. SANDiff与LEACE比较mixed。
5. full-pool相对gain扩大主要来自RAW leakage增强。
6. current LEACE-private channel没有消除retained nonlinear leakage。
```

**信息论桥接：**

```text
Total subject information
=
head-transmitted information
+
head-fiber information
```
...
> Can a stochastic channel operating only in the exact task-head fiber reduce subject leakage while leaving every fixed EEG decision unchanged, and can diffusion preserve the pooled fiber distribution better than its conditional-mean one-step counterpart?
...
second dataset
foundation encoder
full CMI/operator program
nonlinear fiber geometry
