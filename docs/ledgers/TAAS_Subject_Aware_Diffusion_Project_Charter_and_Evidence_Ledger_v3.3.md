...
**版本：** v3.3  
**状态日期：** 2026-08-13（V34P审阅完成；V35P fiber-channel validation路线冻结）  
**建议仓库路径：**
...
> Remove source-fiber subject information while preserving the declared EEG task function exactly.

研究对象是冻结EEG representation：
...
允许替换的部分:
task-head fiber coordinates
```

“Subject information”仍是操作性概念，不解释其唯一生理、session或acquisition来源。

## 2.2 当前核心原则

```text
Preserve the declared function exactly.
Resample only within its fiber.
```

Primary strong channel必须满足：

- centered logits逐样本不变；
- softmax逐样本不变；
- predicted class逐样本不变；
- replacement不读取原始source fiber；
- replacement不读取subject ID；
- output fiber只依赖 \(H\) 与population randomness。
...
```text
比one-step删除更多理想subject information
```

因为strong exact-fiber methods都达到同一个 \(H\)-visible privacy boundary。

Diffusion承担：

```text
学习pooled p(U|H)
保持fiber distribution
避免conditional-mean collapse
提供population-plausible stochastic releases
```

## 2.4 当前不作为主线的内容

```text
waveform ocular operator
latency optimization
identity/session causal decomposition
full CMI estimator
multiscale Fisher operator
nonlinear task manifold
support-specific channel
formal anonymity
```
...
| 层级 | 问题 | 当前状态 |
|---|---|---|
| P0 | exact linear-head fiber geometry是否正确 | **V34P 已建立** |
| P1 | fixed task output是否逐样本不变 | **V34P 已建立：0 mismatch / 0 BA change** |
| P2 | strong channel是否不再读取source fiber | **代码结构成立；V35P做closure audit** |
| P3 | registered attacks相对RAW是否下降 | **V34P 已建立** |
| P4 | output leakage是否与H-only boundary一致 | **有限攻击结果未闭合；V35P待修正** |
| P5 | diffusion是否优于deterministic conditional mean的distribution fidelity | **V34P 已建立** |
| P6 | diffusion是否优于简单stochastic resampling | **未测试** |
| P7 | 更大participant cohort是否复现 | **后续轮次** |
| P8 | 是否形成最终TAAS修回证据 | **未建立** |

V34P已经支持一个正向exact-fiber方法，但diffusion-specific证据还需要一个强
stochastic baseline，而不是继续新增神经网络家族。


# 4. 当前正向方法：Fiber-SANDiff

## 4.1 Exact head-fiber decomposition
...
## 4.2 Strong source-fiber removal channel

Strong Fiber-OneStep：

\[
U'=f(H).
\]

Strong Fiber-SANDiff：

\[
U'=g(H,\xi),\qquad \xi\perp(S,U)\mid H.
\]

二者都不读取source \(U\)。因此在部署channel固定后：
...
该结果是channel dependency的结构结论，不依赖有限attacker是否达到Bayes optimum。

## 4.3 One-step与diffusion

Fiber-OneStep近似conditional mean，适合point estimation，但会压缩fiber variance。

Fiber-SANDiff近似pooled conditional law，目标是：

```text
exact H preservation
+
source-fiber removal
+
population distribution fidelity
```

## 4.4 当前必须加入的stochastic baseline

```text
Fiber-Stratified-Resample
```

从training population中，仅依据：

```text
predicted class from H
+
H-confidence stratum
```

采样一个fiber coordinate。

它：

- 不读取query source fiber；
- 不读取query subject；
- exact-preserves \(H\)；
- 是简单但很强的population stochastic baseline。

## 4.5 Privacy reporting

Primary privacy不再按有限攻击器直接给exact-fiber methods排名。

必须分开：

```text
head-visible leakage:
attack(H)

full-output finite attack:
attack(Z')

head-aware attack:
attack(H, Z')

incremental fiber leakage:
attack(H,U') versus attack(H)
```

Strong exact-fiber channel的理论目标是：

```text
incremental source-fiber leakage = 0
```

而不是让有限attacker数字低于H-only。
...
V34P frozen EEGNet
V34P frozen Fiber-OneStep
V34P frozen Fiber-SANDiff
same outer folds and seeds
```
...
```text
Fiber-Stratified-Resample
head-aware attackers
multi-sample distribution diagnostics
```

## 5.1 必须回答

1. \(H\) 是否能从每个released representation数值恢复；
2. head-aware attacker是否消除“低于H-only”的有限攻击假象；
3. strong output相对H是否仍有增量fiber leakage；
4. Fiber-SANDiff是否优于Fiber-OneStep的distribution fidelity；
5. Fiber-SANDiff是否优于Fiber-Stratified-Resample；
6. stochastic releases是否保持conditional diversity；
7. retrained task information如何变化。

## 5.2 Latency

```text
不运行新的latency benchmark。
不将latency用于方法选择。
```
...
V35P完成语义与baseline闭环后，才进入更大participant cohort。
...
1. 冻结V34P模型与outputs；
2. head-aware privacy accounting；
3. conditional fiber leakage closure；
4. Fiber-Stratified-Resample baseline；
5. repeated-sample conditional distribution diagnostics；
6. corrected method positioning；
7. 不运行latency；
8. 不增加dataset或model family；
9. 结果后升级ledger v3.4。
...
| V32P | `2b1522e7...` | frozen | first SANDiff pilot |
| V33P | `5292c1a5...` | frozen | full-pool SANDiff consolidation |
| V34P | `e10dd401...` | latest complete | exact head-fiber positive method |
| V35P | planned | active | privacy semantics and stochastic baseline validation |
...
不立即扩展更大cohort，原因不是V34P不成立，而是必须先修正diffusion-specific
解释。
...
source-fiber removal at strong endpoint:
由channel architecture成立

privacy reduction versus RAW:
成立

Fiber-SANDiff比OneStep删除更多理想隐私:
不成立；二者共享H-visible boundary

Fiber-SANDiff更好保持fiber distribution:
成立

Fiber-SANDiff优于简单stochastic resampling:
未知
```

V35P只补最后一个关键baseline与head-aware attack closure。

V35P后：

```text
SANDiff优于Resample:
冻结Fiber-SANDiff，扩展更大participant cohort

SANDiff与Resample等价:
保留exact fiber population-resampling方法，
将diffusion定位为learned amortized implementation

Resample明显更好:
不否定exact fiber主线，
但删除diffusion-specific superiority claim
```
...
```text
Exact Head-Fiber Population Resampling
implemented by Fiber-SANDiff
```

**当前最重要的事实：**

```text
1. centered logits、softmax和fixed decision逐样本保持。
2. strong replacement不读取source fiber。
3. strong OneStep与SANDiff的理想subject information均等于H-visible information。
4. 当前有限attacker低于H-only不是额外信息删除证据。
5. SANDiff显著减少conditional-mean variance collapse。
6. diffusion-specific价值目前是distribution fidelity。
7. 简单stochastic resampling baseline尚未运行。
8. latency不参与科学选择。
```

**当前下一问题：**

> Does Fiber-SANDiff preserve the conditional head-fiber distribution better than a simple training-population resampling channel once privacy is evaluated with a head-aware threat model?
