...
> Preserve the declared EEG task function exactly, remove source-fiber information beyond that function, and learn an exemplar-free population fiber channel.
...
Strong OneStep、Resample、SANDiff若只依赖：
...
且不读取source fiber，则其source-subject information都达到同一个H-visible boundary。

因此论文不再比较它们谁“删除更多理想source identity”。
...
model-only
exemplar-free
continuous conditional population sampler
```

需要与两类基线区分：

```text
conditional mean / Gaussian:
model-only但分布能力有限

stratified resampling:
经验分布强，但需要training-fiber bank并直接释放exemplar
```
...
waveform ocular cleaning
new identity/session causal decomposition
full mutual-information estimator
formal anonymity
new diffusion architecture family
```
...
| 层级 | 问题 | 当前状态 |
|---|---|---|
| P0 | exact linear-head fiber geometry | **V34P/V35P 已建立** |
| P1 | source-fiber removal与H-visible boundary | **V35P 已建立** |
| P2 | BCI-IV-2a registered attacks与distribution结果 | **已建立，但n=9、单数据集** |
| P3 | diffusion优于empirical resampling | **当前否定** |
| P4 | diffusion优于model-only Gaussian/mean baseline | **外部cohort待验证** |
| P5 | diffusion减少training-exemplar exposure | **待验证** |
| P6 | 54-participant external replication | **V36P待运行** |
| P7 | 第二任务/encoder generalization | **后续** |
| P8 | 最终TAAS修回证据 | **未建立** |

V36P不是继续在9名participants上扩大ablation，而是一次预先冻结的external-cohort
replication。


# 4. 当前正向方法：Exemplar-Free Fiber-SANDiff

## 4.1 Exact fiber channel

\[
H=CZ,\qquad Z=Z_H+NU,\qquad CN=0.
\]
...
近似conditional mean，易variance collapse。

### Fiber-Gaussian

\[
U'\sim\mathcal N(\mu(H),\Sigma(H)).
\]

model-only stochastic baseline，用于检验简单参数分布是否足够。

### Fiber-Stratified-Resample

\[
U'=U_J,\qquad J\sim\text{training bank conditioned on }H.
\]

它是bank-dependent empirical reference，不再自动视为最终privacy method。

### Fiber-SANDiff

\[
U'\sim q_\theta(U\mid H).
\]

部署时不保留training fibers；目标是拟合非高斯conditional population law，并减少
training exemplar release。

## 4.3 两种privacy对象

### Query-source privacy

所有strong source-independent fiber channels共享H-visible boundary。

### Training-population exposure

需要额外评价：

```text
exact / near copy
nearest training-fiber distance
training-donor attribution
membership / exposure attack
```

这是Resample与model-only sampler真正不同的地方。

## 4.4 正向方法主张

期望主张：

> Fiber-SANDiff realizes an exact function-preserving, model-only population fiber channel that reduces source-subject leakage without releasing stored training exemplars.

该主张必须由OpenBMI external replication支持。
...
```text
OpenBMI External-Cohort Replication
and Exemplar-Free Fiber Diffusion
```
...
```text
OpenBMI / Lee2019 motor imagery
54 participants
2 sessions
one EEGNet encoder
exact linear-head fiber
2 seeds
...
2. head-aware source privacy；
3. conditional distribution fidelity；
4. multi-sample diversity；
5. training-exemplar exposure；
6. retrained utility；
7. participant heterogeneity。

V36P不改变Fiber-SANDiff architecture，不新增第二encoder或第二task。

V36P之后：

```text
若external replication与exemplar-free优势成立:
冻结方法，开始稿件与第二任务补充

若Gaussian与SANDiff等价:
保留exact model-only stochastic channel，收窄diffusion claim

若只有bank resampling有效:
exact fiber方法成立，但当前diffusion主线不再具备实证优势
```
...
1. OpenBMI 54-participant external cohort；
2. same exact-fiber geometry；
3. model-only Fiber-Gaussian baseline；
4. bank-dependent Resample reference；
5. frozen Fiber-SANDiff architecture；
6. head-aware source privacy；
7. training-exemplar exposure；
8. participant-first external replication；
9. no latency gate；
10. 结果后升级ledger v3.6。
...
|---|---|---|---|
| A-track | `0c4f2301...` | 只读 | waveform历史理论 |
| V31 | `274b371e...` | frozen | Route P transition |
| V32P | `2b1522e7...` | frozen | first representation privacy pilot |
| V33P | `5292c1a5...` | frozen | full-pool consolidation |
| V34P | `e10dd401...` | frozen | exact head-fiber method |
| V35P | `096b43fc...` | latest complete | head-aware closure and resample baseline |
| V36P | planned | active | OpenBMI external replication and exemplar-free claim |
...
```text
V36P OpenBMI external replication
```

## 12.1 为什么不能继续只做BCI-IV-2a

```text
participant n = 9
one task
one encoder
same dataset used across multiple method rounds
```

exact theorem仍然可信，但经验privacy与distribution结论需要独立cohort。

## 12.2 为什么仍然保留diffusion

V35P已经否定：

```text
SANDiff beats empirical resampling on BCI-IV-2a distribution fidelity
```

V36P检验新的、预先登记的差异：

```text
bank-dependent exemplar release
vs
model-only amortized sampling
```

这不是重新解释V35P，而是补充training-population privacy与deployment-state问题。

## 12.3 结果纪律

V36P不允许在OpenBMI结果后修改：
...
若结果不支持SANDiff，不再在同一cohort上搜索新架构。
...
```text
exact function-preserving population fiber replacement
```

**当前被撤回的主张：**

```text
Fiber-SANDiff empirically superior to simple stratified resampling
```

**当前保留的diffusion假设：**

```text
Fiber-SANDiff may provide an exemplar-free,
model-only approximation to the population fiber channel
with less training-data exposure than bank resampling.
```

**当前经验可信度：**

```text
BCI-IV-2a internal evidence:
moderate, dataset-specific, n=9

exact geometry and channel dependency:
high
...
```text
V36P OpenBMI, n=54, two sessions
```
...
new diffusion family
latency gate
waveform sealed data
