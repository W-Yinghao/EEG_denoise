...
**版本：** v2.5  
**状态日期：** 2026-08-13（AE许可确认；主线切换至privacy-aware representation denoising）  
**建议仓库路径：**
...
从 training users 与 query-disjoint support 得到 private subspace：

\[
P_s,\qquad Q_s=I-P_s.
\]

只在 private span 中前向加噪：
...
```text
Q_s Z
task label / teacher logits / task prototype
acquisition metadata if necessary
```

但不保留 persistent subject token。
...
```text
privacy:
reduce adaptive identity/linkage attack performance
...
External representation setting建议：

```text
one sleep-staging dataset
or one EEG foundation-model benchmark cohort
```
...
open-set verification / linkage
cross-session linkage
adaptive attacker retrained after sanitization
membership inference
optional protected-attribute inference
```
...
```text
V31R — Privacy Representation Feasibility and Baseline Reproduction
```

本轮先做：

1. two datasets × two encoders；
2. RAW / LEACE / DANN / perturbation / ID-RemovalNet-style baselines；
3. closed-set + open-set privacy attacks；
4. fixed-head + retrained-head utility；
5. private-subspace diffusion K=1 pilot；
6. matched one-step sanitizer；
7. privacy–utility Pareto；
8. V27 waveform preprocessing作为输入条件；
9. GPU-first，少hard gates；
10. 不打开V30 sealed confirmation。
