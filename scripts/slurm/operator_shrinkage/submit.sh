#!/bin/bash
set -euo pipefail
stage=$1;profile=${2:-cpu};dep=${3:-};task=${4:-};ROOT=/home/infres/yinwang/denoiseNet_operator_shrinkage
...
case "$profile" in
 ...
 gpu1to8) p=A100,H100,L40S;c=8;m=64G;t=08:00:00;extra=(--gres=gpu:1 --array=1-8%8) ;;
...
job=$(sbatch "${args[@]}" "$ROOT/scripts/slurm/operator_shrinkage/job.sbatch" "$stage");meta=$dep;[[ -n "$task" ]]&&meta="${meta:+$meta;}task=$task";printf '%s|%s|%s|%s|%s\n' "$(date -u +'%FT%TZ')" "$job" "$profile" "$stage" "$meta">>"$ROOT/reports/slurm/operator_shrinkage_job_ids.txt";echo "$job"
