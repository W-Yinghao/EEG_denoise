...
mkdir -p "$ROOT/slurm_logs/sge_score_lora_v8" "$ROOT/reports/slurm"
args=(--parsable --partition="$p" --cpus-per-task="$c" --mem="$m" --time="$t" --job-name="v8_${stage}" --output="$ROOT/slurm_logs/sge_score_lora_v8/%x_%A_%a.out" --error="$ROOT/slurm_logs/sge_score_lora_v8/%x_%A_%a.err" --export="$export_vars")
[[ -z "$g" ]] || args+=(--gres="$g")
[[ -z "$after" ]] || args+=(--dependency="afterok:$after")
[[ -z "$array" ]] || args+=(--array="$array")
job=$(sbatch "${args[@]}" "$ROOT/scripts/slurm/sge_score_lora_v8/job.sbatch" "$stage")
printf '%s|%s|%s|%s\n' "$(date -u +'%Y-%m-%dT%H:%M:%SZ')" "$job" "$profile" "$stage" >> "$ROOT/reports/slurm/sge_score_lora_v8_job_ids.txt"
printf '%s\n' "$job"
