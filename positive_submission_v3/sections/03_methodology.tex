...
\begin{equation}
  ...
  \label{eq:interval}
\end{equation}
where $\tau_{-f}$ is a scalar temperature chosen without the evaluated fold, as the smallest value on a grid from 0.50 to 6.00 in steps of 0.05 reaching 80\% coverage on the remaining folds, and $z_p$ is the standard-normal quantile. For the held-out cohort, one scalar per policy is frozen on all development cells jointly by the same grid rule and applied unchanged. The interval $I_{1-\alpha}$ is evaluated empirically, by marginal coverage, width, the continuous ranked probability score, and risk--coverage curves, in Section~\ref{sec:results}.

\subsection{The adaptation loop}
\label{sec:method-loop}

Assembled, the stages form a small adaptation loop rather than a fixed network. The system \emph{monitors} its own calibration through the block-agreement statistic $v_i$; \emph{analyzes} it into a shrinkage weight $\lambda_i$ and a binary reliability verdict; \emph{plans} by choosing between the calibrated guide and the withheld-guide fallback; and \emph{executes} by guided sampling that also reports its remaining uncertainty. Two constants of this loop are measured rather than assumed. The first is the floor: below 60\,s the four sub-block statistics that drive both shrinkage and the reliability verdict are undefined, so the rule rejects short calibrations by construction --- and Section~\ref{sec:res-loop} shows that 60\,s already delivers 97\% of the two-minute gain, so the floor sits exactly where the information budget allows it to. The second is the clock: the propagation matrix re-estimated on later windows of the same recording drifts away from the calibration estimate, and the natural-recording benefit decays on the same horizon, so the loop's natural corrective action is a fresh two-minute calibration, not on-line tracking --- which Section~\ref{sec:res-natural} shows never reaches the calibrated operating point.
