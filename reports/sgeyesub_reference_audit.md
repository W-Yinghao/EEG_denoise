...
5. Support EOG and EOG-derived labels may be used for participant calibration. Query EOG/labels may be used only for an explicitly external-reference evaluation; they must not affect fitting, selection, gating, or stopping. Native correction itself is the frozen matrix application.
