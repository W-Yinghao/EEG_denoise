...
J0 completed on Slurm jobs 924788, 924795, 924869, and the corrected retry
924883. All 13 named author
...
| EEGOAR-Net | `4e67cb9` | Official TensorFlow architecture, pretrained `h5` weights, channel standardization helpers, and an example with three subjects are present; no official training/split entry is included. The Slurm runtime audit in the registered GPU environment stopped at `ModuleNotFoundError: tensorflow` before model construction. | Official pretrained runtime is blocked by the registered environment dependency, while frozen-split retraining is independently unavailable from the repository. This is not counted as a negative model result. |
...
| ART | `2ff4ff3` | Official transformer source and a training entry are available; its supervised target is constructed from ICA-labelled source mixtures. | Source verified; not silently substituted for the frozen paired clean-target protocol. |
| IC-U-Net | `7f4f27d` | Public PyTorch inference/training surface and fixed-channel example are available; the target is an ICA brain/non-brain source decomposition. | Source verified; target/channel contract requires an explicit port rather than an exact frozen-split claim. |
| ResTL | `1f05ad2` | Official cross-subject MI code uses target resting-state EEG to adapt a classifier. | Mechanism evidence for raw-support adaptation, not artifact-waveform restoration. |
| TSMNet | `90293b9` | Official domain-specific SPD normalization/classification implementation is complete enough for source audit. | Motivation for the P-D support-statistic control, not a denoising baseline. |
