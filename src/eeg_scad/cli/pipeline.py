...
def sanity(run:Path)->dict[str,Any]:
    cfg=_load("scad_canonical");data=load_training_split(DERIVED,0,"train");device=torch.device("cuda");idx=np.arange(4);x=torch.from_numpy(data["x"][idx]).to(device);y=torch.from_numpy(data["y"][idx]).to(device);a=torch.from_numpy(data["artifact"][idx]).to(device);cm=torch.from_numpy(data["context_match"][idx]).to(device);cw=torch.from_numpy(data["context_wrong"][idx]).to(device)
    ...
    for name,model in (("DET",det),("SCAD",diff)):
        ...
        for step in range(180):
            ...
            if name=="DET":pred=model(y,cm);wrong=model(y,cw);base=(pred-a).square().mean();identity=model(x,cm).square().mean()
            else:base,extra=model.training_loss(a,y,cm,gen,timestep=torch.full((len(y),),500,device=device,dtype=torch.long));pred=extra["predicted_x0"];wrong=model.predict(extra["state"],y,extra["timestep"],cw)[1];identity,_=model.training_loss(torch.zeros_like(a),x,cm,gen)
            rank=ranking_loss(pred,wrong,a,.01);loss=base+.1*rank+identity
        ...
        results[name]={"initial_loss":initial,"final_loss":final,"loss_reduction":1-final/max(initial,1e-12),"context_output_change":float((torch.linalg.vector_norm(pred-wrong)/torch.linalg.vector_norm(pred).clamp_min(1e-8)).detach()),"all_gradients_finite_nonzero":True,"parameters":sum(p.numel() for p in model.parameters())}
    zero=torch.zeros_like(a);det_zero=det(x,cm);noise=torch.randn(a.shape,device=device,generator=torch.Generator(device=device).manual_seed(88));diff_zero,trace=diff.sample(x,cm,noise,25,True);input_rms=float(x.square().mean().sqrt());results["zero_identity"]={"DET_artifact_rms":float(det_zero.square().mean().sqrt().detach()),"SCAD_artifact_rms":float(diff_zero.square().mean().sqrt()),"input_rms":input_rms,"DET_relative_change":float((det_zero.square().mean().sqrt()/x.square().mean().sqrt().clamp_min(1e-8)).detach()),"SCAD_relative_change":float(diff_zero.square().mean().sqrt()/x.square().mean().sqrt().clamp_min(1e-8)),"empirical_envelope":0.5};results["trajectory"]=trace
    clean=x-diff_zero;flat=diff_zero.flatten();results["output_scale"]={"input_rms":input_rms,"target_artifact_rms":float(a.square().mean().sqrt()),"predicted_artifact_rms":float(diff_zero.square().mean().sqrt()),"clean_output_rms":float(clean.square().mean().sqrt()),"artifact_q01":float(torch.quantile(flat,.01)),"artifact_q50":float(torch.quantile(flat,.50)),"artifact_q99":float(torch.quantile(flat,.99)),"artifact_max_abs":float(flat.abs().max()),"channel_variance_ratio":float(torch.var(clean,dim=-1).mean()/torch.var(x,dim=-1).mean().clamp_min(1e-8)),"observation_change_ratio":float(torch.linalg.vector_norm(diff_zero)/torch.linalg.vector_norm(x).clamp_min(1e-8))}
    ...
    continuation=max(float(torch.max(torch.abs(pa-pb)).detach()) for pa,pb in zip(probe_model.parameters(),resumed.parameters()));ema_diff=max(float(torch.max(torch.abs(probe_ema.shadow[k]-resumed_ema.shadow[k])).detach()) for k in probe_ema.shadow);checkpoint.unlink()
    ...
    results["finite"]=all(np.isfinite(v) for v in [results["DET"]["final_loss"],results["SCAD"]["final_loss"]]);results["engineering_checks"]={"DET_fixed_batch_reduction":results["DET"]["loss_reduction"]>.5,"SCAD_fixed_batch_reduction":results["SCAD"]["loss_reduction"]>.5,"DET_context_load_bearing":results["DET"]["context_output_change"]>1e-6,"SCAD_context_load_bearing":results["SCAD"]["context_output_change"]>1e-6,"DET_zero_artifact_identity":results["zero_identity"]["DET_relative_change"]<results["zero_identity"]["empirical_envelope"],"SCAD_zero_artifact_identity":results["zero_identity"]["SCAD_relative_change"]<results["zero_identity"]["empirical_envelope"],"scale_finite_and_bounded":results["output_scale"]["artifact_max_abs"]<100*max(results["output_scale"]["input_rms"],1e-6),"checkpoint_exact":results["checkpoint_reload_max_difference"]==0,"common_noise_exact":results["common_noise_replay_max_difference"]==0,"resume_continuation":max(results["resume"]["raw_reload_max_difference"],results["resume"]["optimizer_continuation_max_difference"],results["resume"]["ema_continuation_max_difference"])<=1e-6};results["stage"]="R5";results["status"]="PASS" if results["finite"] and all(results["engineering_checks"].values()) else "FAIL"
...
def aggregate_report(run:Path)->dict[str,Any]:
    ...
    summary_rows=_read(RESULT/"method_summary.csv");latency_rows=_read(RESULT/"latency_summary.csv")
    def mean(method:str,metric:str)->float:
        return float(next(row["mean"] for row in summary_rows if row["method"]==method and row["metric"]==metric))
    def latency(method:str)->str:
        row=next((value for value in latency_rows if value["method"]==method),None)
        return "not recorded" if row is None else f"{float(row['milliseconds_per_window']):.2f} ms/window, NFE={row['nfe']}, {float(row['peak_memory_mb']):.0f} MiB"
    sources=yaml.safe_load((ROOT/"third_party/source_registry.yaml").read_text())["sources"]
    public_result=json.loads((RESULT/"baseline_reproduction/eegdus_public_result.json").read_text())
    ...
    public_rrmse=[float(v["rrmse_mean"]) for v in public_result["test_summary"]];public_corr=[float(v["correlation_mean"]) for v in public_result["test_summary"]]
    (report_dir/"scad_v22_baseline_reproduction.md").write_text(f"# SCAD V22 baseline reproduction\n\nEEGDfus and D4PM were pinned and audited. Their releases do not contain an explicit license file, so official-native code was not redistributed or presented as an exact reproduction. The clean-room EEGDfus-style public benchmark used 3400 EEG/EOG source records with a source-record-disjoint split, 2000 updates, and 734401 parameters. Across the registered −5 to +5 dB panel, mean RRMSE ranged {min(public_rrmse):.3f}–{max(public_rrmse):.3f} and correlation {min(public_corr):.3f}–{max(public_corr):.3f}. It is classified as `architecture_reimplementation / local_results_reasonable_but_nonidentical`, not an official-native result. D4PM is `blocked_incomplete_release` for an auditable official-native comparison. The matched deterministic multichannel artifact U-Net is the strong local baseline.\n",encoding="utf-8")
    sanity_lines=[f"- DET fixed-batch loss reduction: {sanity_result['DET']['loss_reduction']:.4f}",f"- SCAD fixed-batch loss reduction: {sanity_result['SCAD']['loss_reduction']:.4f}",f"- DET/SCAD parameters: {sanity_result['DET']['parameters']:,} / {sanity_result['SCAD']['parameters']:,}",f"- zero-artifact relative change DET/SCAD: {sanity_result['zero_identity']['DET_relative_change']:.4f} / {sanity_result['zero_identity']['SCAD_relative_change']:.4f}",f"- context response DET/SCAD: {sanity_result['DET']['context_output_change']:.4f} / {sanity_result['SCAD']['context_output_change']:.4f}",f"- checkpoint reload max difference: {sanity_result['checkpoint_reload_max_difference']:.3g}",f"- common-noise replay max difference: {sanity_result['common_noise_replay_max_difference']:.3g}",f"- optimizer/EMA resume max differences: {sanity_result['resume']['optimizer_continuation_max_difference']:.3g} / {sanity_result['resume']['ema_continuation_max_difference']:.3g}"]
    ...
    paired_p=diagnosis["paired_SCAD_MATCH_POP"];paired_w=diagnosis["paired_SCAD_MATCH_WRONG"];diff=diagnosis["paired_SCAD_K1_DET1"];natural_p=diagnosis["natural_SCAD_MATCH_POP"];natural_w=diagnosis["natural_SCAD_MATCH_WRONG"]
    project=("# SCAD V22 project reset\n\n"
      ...
      f"The canonical DET and SCAD models have {sanity_result['DET']['parameters']:,} and {sanity_result['SCAD']['parameters']:,} trainable parameters (SCAD +{100*(sanity_result['SCAD']['parameters']/sanity_result['DET']['parameters']-1):.1f}%). Both passed finite-gradient, fixed-batch overfit, observation-anchor, context-response, trajectory, fixed-noise, checkpoint and interrupted optimizer/EMA/RNG resume checks. EEGDfus official-native and D4PM official-native claims are blocked by their unlicensed/incompletely documented releases; the clean-room public architecture reimplementation gave RRMSE {min(public_rrmse):.3f}–{max(public_rrmse):.3f} and correlation {min(public_corr):.3f}–{max(public_corr):.3f} across −5 to +5 dB.\n\n"
      "## Paired development evidence\n\n"
      f"Participant-first mean temporal RRMSE was {mean('DET_MATCH','rrmse_temporal'):.4f} for DET-MATCH and {mean('SCAD_MATCH','rrmse_temporal'):.4f} for SCAD-MATCH. SCAD MATCH−POP utility was {paired_p['mean']:+.5f} (median {paired_p['median']:+.5f}, {paired_p['positive_count']}/15 positive), and MATCH−WRONG {paired_w['mean']:+.5f} (median {paired_w['median']:+.5f}, {paired_w['positive_count']}/15). The K1 diffusion-minus-DET1 utility was {diff['mean']:+.5f} with {diff['positive_count']}/15 positive; thus DET was better for every participant.\n\n"
      f"Removing context ranking improved mean RRMSE from {mean('SCAD_MATCH','rrmse_temporal'):.4f} to {mean('SCAD_NO_RANK_MATCH','rrmse_temporal'):.4f}. The v-parameterized pilot was worse at {mean('SCAD_V_MATCH','rrmse_temporal'):.4f}. DDIM10/25/50 MATCH RRMSE was {mean('SCAD_MATCH_DDIM10','rrmse_temporal'):.4f}/{mean('SCAD_MATCH','rrmse_temporal'):.4f}/{mean('SCAD_MATCH_DDIM50','rrmse_temporal'):.4f}; iterative step effects were small and non-monotonic.\n\n"
      "## Natural development evidence\n\n"
      f"For held-out EOG remaining ratio (lower is better), SCAD-MATCH was {mean('SCAD_MATCH','heldout_eog_remaining_ratio'):.4f}, compared with DET-MATCH {mean('DET_MATCH','heldout_eog_remaining_ratio'):.4f}. SCAD natural MATCH−POP utility was {natural_p['mean']:+.5f} ({natural_p['positive_count']}/15), and MATCH−WRONG {natural_w['mean']:+.5f} ({natural_w['positive_count']}/15). MATCH preservation was {mean('SCAD_MATCH','preservation'):.4f}; relative to SCAD-POP its preservation utility was {diagnosis['natural_preservation_MATCH_POP']['mean']:+.5f}, PSD utility {diagnosis['natural_PSD_MATCH_POP']['mean']:+.5f}, and covariance utility {diagnosis['natural_covariance_MATCH_POP']['mean']:+.5f}.\n\n"
    ...
    natural=(f"# SCAD V22 natural development evaluation\n\nNatural SGEYESUB results are reported as an attenuation–preservation trade-off. Query EOG and query operators were absent from inference and opened only after the 15-file output freeze. Natural data have no paired clean counterfactual, so attenuation alone is not interpreted as successful denoising.\n\nSCAD-MATCH had held-out EOG remaining ratio {mean('SCAD_MATCH','heldout_eog_remaining_ratio'):.4f}, preservation {mean('SCAD_MATCH','preservation'):.4f}, PSD distortion {mean('SCAD_MATCH','psd_distortion'):.4f}, and covariance distortion {mean('SCAD_MATCH','covariance_distortion'):.4f}. DET-MATCH remaining ratio was {mean('DET_MATCH','heldout_eog_remaining_ratio'):.4f}. Relative to SCAD-POP, MATCH slightly improved attenuation ({natural_p['mean']:+.5f}) but reduced preservation ({diagnosis['natural_preservation_MATCH_POP']['mean']:+.5f}) and did not improve PSD/covariance on average. The resulting label is `{diagnosis['natural_EEG_tradeoff']}`.\n")
