...
def aggregate_natural(run_id: str) -> None:
    ...
    pivot = pd.DataFrame(summary).pivot(index="condition", columns="metric", values="participant_mean").reset_index(); fig, ax = plt.subplots(figsize=(6,4)); ax.scatter(pivot.artifact_attenuation_db, pivot.low_eog_observation_retention); [ax.annotate(row.condition, (row.artifact_attenuation_db,row.low_eog_observation_retention)) for row in pivot.itertuples()]; fig.tight_layout(); fig.savefig(FIGURE/"natural_attenuation_retention.png",dpi=180); plt.close(fig)


def figures(summary, effects, duration):
    rr = summary[(summary.panel == "paired") & (summary.metric == "rrmse_temporal")]
    fig, ax = plt.subplots(figsize=(7,4)); ax.bar(rr.condition, rr.participant_mean); ax.tick_params(axis="x", rotation=25); ax.set_ylabel("Temporal RRMSE"); fig.tight_layout(); fig.savefig(FIGURE/"official_vs_paired_population.png",dpi=180); plt.close(fig)
    primary = effects[effects.metric == "rrmse_temporal"]; fig, ax = plt.subplots(figsize=(7,4)); ax.bar(primary.contrast, primary.participant_mean_utility); ax.axhline(0,color="black",lw=.8); ax.tick_params(axis="x",rotation=25); fig.tight_layout(); fig.savefig(FIGURE/"transfer_condition_effect.png",dpi=180); plt.close(fig)
    dur = duration.groupby(["support_seconds","participant"],as_index=False).mean(numeric_only=True).groupby("support_seconds",as_index=False).mean(numeric_only=True); fig, ax=plt.subplots(figsize=(6,4)); ax.plot(dur.support_seconds,dur.rrmse_temporal,marker="o"); ax.set(xlabel="Support seconds",ylabel="Temporal RRMSE"); fig.tight_layout(); fig.savefig(FIGURE/"support_duration.png",dpi=180); plt.close(fig)
    oracle = effects[(effects.metric=="rrmse_temporal") & effects.contrast.isin(["MATCH-POP","MATCH-ORACLE"])]; fig,ax=plt.subplots(figsize=(5,4)); ax.bar(oracle.contrast,oracle.participant_mean_utility); ax.axhline(0,color="black",lw=.8); fig.tight_layout(); fig.savefig(FIGURE/"oracle_headroom.png",dpi=180); plt.close(fig)
    fig,ax=plt.subplots(figsize=(7,4)); ax.bar(primary.contrast,primary.positive_count); ax.set(ylabel="Positive participants"); ax.tick_params(axis="x",rotation=25); fig.tight_layout(); fig.savefig(FIGURE/"participant_effects.png",dpi=180); plt.close(fig)
