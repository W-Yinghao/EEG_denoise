...
def aggregate(run:Path)->dict[str,Any]:
    ...
    for kind in ("v22_fixed","of_det","pop_marginal_det","of_scad","pop_marginal_scad"):
        ...
        if records:exposure.append({"model":kind,"parameters":records[0].get("parameters"),"anchor_parameters":records[0].get("anchor_parameters",0),"runs":len(records),"updates_min":min(int(r["updates"]) for r in records),"updates_max":max(int(r["updates"]) for r in records),"updates_mean":float(np.mean([int(r["updates"]) for r in records])),"training_seconds_total":float(np.sum([float(r.get("training_seconds",0)) for r in records])),"online_mixtures_per_update":int(_cfg(kind).get("batch_size",16)),"checkpoint_rule":"best_sampling" if "scad" in kind or kind=="v22_fixed" else "best_artifact"})
    ...
    def method_mean(rows:list[dict[str,Any]],method:str,metric:str)->float:return float(np.mean([float(r[metric]) for r in rows if r["method"]==method]))
    round_b_report=("# V23 Round B\n\nAll five development folds and three training seeds completed for OF-DET, OF-SCAD-K1, POP-MARGINAL-DET and POP-MARGINAL-SCAD-K1. Seeds were averaged within participant before the canonical n=15 summary.\n\n"
      f"- Paired clean RRMSE: OF-DET-MATCH {method_mean(paired,'OF_DET_MATCH','rrmse_temporal'):.6f}; POP-MARGINAL-DET {method_mean(paired,'POP_MARGINAL_DET','rrmse_temporal'):.6f}; OF-SCAD-K1-MATCH {method_mean(paired,'OF_SCAD_K1_MATCH','rrmse_temporal'):.6f}; POP-MARGINAL-SCAD-K1 {method_mean(paired,'POP_MARGINAL_SCAD_K1','rrmse_temporal'):.6f}.\n"
      f"- Artifact-field RRMSE: OF-DET-MATCH {method_mean(paired,'OF_DET_MATCH','artifact_rrmse'):.6f}; POP-MARGINAL-DET {method_mean(paired,'POP_MARGINAL_DET','artifact_rrmse'):.6f}; OF-SCAD-K1-MATCH {method_mean(paired,'OF_SCAD_K1_MATCH','artifact_rrmse'):.6f}; POP-MARGINAL-SCAD-K1 {method_mean(paired,'POP_MARGINAL_SCAD_K1','artifact_rrmse'):.6f}.\n"
      "- Canonical effects (positive favors MATCH/diffusion): "+", ".join(f"{k}={v:+.6f}" for k,v in collapsed.items())+".\n"
      f"- K1 stability: seed means {json.loads((RESULT/'round_b/k_decision.json').read_text())['seed_means']}; K8/DET8 was not authorized because only one of three seeds favored diffusion.\n\n"
    ...
    natural_report=("# V23 natural development\n\nInference used EEG plus frozen support/population context only. Query EOG was opened by the evaluator after a 15-output digest freeze; query-EOG inference reads and sealed reads were both zero.\n\n"
      f"- OF-SCAD-MATCH: remaining-ratio {method_mean(natural,'OF_SCAD_K1_MATCH','heldout_eog_remaining_ratio'):.6f}, attenuation {method_mean(natural,'OF_SCAD_K1_MATCH','artifact_attenuation_db'):.6f} dB, preservation {method_mean(natural,'OF_SCAD_K1_MATCH','preservation'):.6f}, PSD distortion {method_mean(natural,'OF_SCAD_K1_MATCH','psd_distortion'):.6f}, covariance distortion {method_mean(natural,'OF_SCAD_K1_MATCH','covariance_distortion'):.6f}.\n"
      f"- POP-MARGINAL-SCAD: remaining-ratio {method_mean(natural,'POP_MARGINAL_SCAD_K1','heldout_eog_remaining_ratio'):.6f}, attenuation {method_mean(natural,'POP_MARGINAL_SCAD_K1','artifact_attenuation_db'):.6f} dB, preservation {method_mean(natural,'POP_MARGINAL_SCAD_K1','preservation'):.6f}, PSD distortion {method_mean(natural,'POP_MARGINAL_SCAD_K1','psd_distortion'):.6f}, covariance distortion {method_mean(natural,'POP_MARGINAL_SCAD_K1','covariance_distortion'):.6f}.\n"
      f"- Subject-vs-population attenuation utility {nat_att:+.6f}; preservation utility {nat_pre:+.6f}. Classification: `{nat_label}`. ERP/SSVEP proxies follow the same low-artifact preservation endpoint and are fully listed in `method_summary.csv`.\n")
    reports=ROOT/"reports";reports.mkdir(exist_ok=True);(reports/"v23_final_development_diagnosis.md").write_text(report);(reports/"v23_round_b.md").write_text(round_b_report);(reports/"v23_natural_development.md").write_text(natural_report);(reports/"v23_project_plan.md").write_text("# V23 project plan\n\nThe executed backbone is context-consistent online counterfactual training plus an operator-factorized spatial decoder, deterministic coefficient anchor, and low-dimensional residual diffusion. Energy guidance and sealed confirmation are out of scope.\n")
