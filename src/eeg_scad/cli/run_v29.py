...
def train_stage(stage,run):
    ...
    if stage=="r4-rounda":kind,variant=ROUND_A[index//2];fold=(0,2)[index%2];seed=20260905
        ...
        kinds=("support_adapter_det","pop_adapter_det","support_adapter_cdm","pop_adapter_cdm");kind=kinds[index//15];cell=index%15;variant="selected";fold=cell//3;seed=SEEDS[cell%3]
...
def _lineage():
    rows=[];by={};explicit_superseded={"939027","939028","939029","939030","939031","939032","939033","939038","939039","939040","939045","939218"}
    for job in sorted((RESULT/"runs").glob("*/job_*")):
        for path in sorted(job.glob("task_*")) or [job]:
            stage=path.parts[-3] if path.name.startswith("task_") else path.parent.name;job_id=job.name.removeprefix("job_");task=path.name.removeprefix("task_") if path.name.startswith("task_") else "";status="accepted" if (path/"result_summary.json").is_file() or (path/"pytest.txt").is_file() else "failed";status="superseded" if job_id in explicit_superseded else status;row={"stage":stage,"job_id":job_id,"array_task":task,"status":status,"recovery_of":"","scientific_setting_changed":False};rows.append(row);by.setdefault((stage,task),[]).append(row)
    # Dependency-cancelled jobs never entered the job script and therefore
    # have no run directory; retain them explicitly in the provenance ledger.
    manual=(("r2-prepare","939027"),("r3-sanity","939028"),("r4-rounda","939029"),("r5-rounda-eval","939030"),("r7-select","939031"),("r4-rounda","939038"),("r5-rounda-eval","939039"),("r7-select","939040"),("r7-select","939045"),("r9-paired-eval","939218"))
    existing={(r["stage"],r["job_id"]) for r in rows}
    rows.extend({"stage":stage,"job_id":job,"array_task":"","status":"superseded","recovery_of":"","scientific_setting_changed":False} for stage,job in manual if (stage,job) not in existing)
    for cell in by.values():
        failed=[r for r in cell if r["status"] in ("failed","superseded")];accepted=[r for r in cell if r["status"]=="accepted"]
...
def package(run):
    ...
    diagnosis=json.loads((RESULT/"development_diagnosis.json").read_text());ledger=ROOT/"docs/TAAS_SUBJECT_AWARE_DIFFUSION_PROJECT_LEDGER.md";queue=subprocess.check_output(["squeue","--me","--noheader","-o","%i %j %T"],text=True);counts={s:sum(r["status"]==s for r in lineage) for s in ("accepted","failed","superseded","recovery")};value={"protocol_id":"population_anchored_support_adapter_clean_diffusion_v29","development_only":True,"base_commit":BASE,"implementation_commit":"65b535edf07d134f242a5a215b84526213a7a750","forensic_commit":"3f595c6160be7a50651b5486a15d34b11089688c","round_a_commit":"63d9c24","round_b_commit":"1fda40d5bbd2b69656b4e0ea2ea910222acc87eb","natural_result_commit":"fc7abf06546fdf0f2ea7baefda4c76360f0ead2c","ledger_v2_0_commit":"624978018df74a7f22f4353b9571986814a274f6","report_package_commit":subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip(),"terminal_commit":"SELF_REFERENTIAL_REPORTED_EXTERNALLY","push_status":"push_verified_after_terminal_commit","remote_sha":"reported_after_push","model_cells":60,"checkpoint_bindings":60,"paired_inference_outputs":15,"natural_inference_outputs":15,"targeted_tests":count("r15-tests"),"clean_archive_tests":count("r16-clean"),"job_status_counts":counts,"accepted_jobs":[r["job_id"] for r in lineage if r["status"]=="accepted"],"failed_jobs":[r["job_id"] for r in lineage if r["status"]=="failed"],"superseded_jobs":[r["job_id"] for r in lineage if r["status"]=="superseded"],"recovery_jobs":[{"job_id":r["job_id"],"recovery_of":r["recovery_of"]} for r in lineage if r["status"]=="recovery"],"current_v29_jobs":[line for line in queue.splitlines() if "v29_" in line],"query_EOG_inference_reads":0,"query_operator_inference_reads":0,"event_inference_reads":0,"sealed_reads":0,"A_track_head":"0c4f2301c1f873120fe54537cde3c76fff7ea3a2","A_track_unchanged":True,"manuscript_unchanged":True,"K":1,"gpu_environment":"icml","cpu_environment":"eeg2025","project_ledger_path":"docs/TAAS_SUBJECT_AWARE_DIFFUSION_PROJECT_LEDGER.md","project_ledger_version":"v2.0","project_ledger_sha256":_digest(ledger),**diagnosis};_json(RESULT/"terminal_manifest.json",value);_json(run/"result_summary.json",value);return value


STAGES={"r0-preflight":preflight,"r1-forensic":forensic,"r2-prepare":prepare,"r3-sanity":sanity,"r4-rounda":lambda r:train_stage("r4-rounda",r),"r5-rounda-eval":round_a_eval,"r7-select":select,"r8-all":lambda r:train_stage("r8-all",r),"r8-support-adapter-det":lambda r:train_stage("r8-support-adapter-det",r),"r8-pop-adapter-det":lambda r:train_stage("r8-pop-adapter-det",r),"r8-support-adapter-cdm":lambda r:train_stage("r8-support-adapter-cdm",r),"r8-pop-adapter-cdm":lambda r:train_stage("r8-pop-adapter-cdm",r),"r9-paired-infer":paired_infer,"r9-paired-eval":paired_eval,"r10-natural-infer":natural_infer,"r11-freeze":freeze_outputs,"r12-natural-eval":natural_eval,"r13-aggregate":aggregate,"r14-ledger":ledger_check,"r17-package":package}
def main():
