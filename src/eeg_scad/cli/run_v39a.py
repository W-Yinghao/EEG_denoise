...
def aggregate():
    ...
    def summarize(frame,metrics):
        ...
        for method,b in per.groupby("method"):
            for m in metrics:
                lo,hi=bootstrap(b[m]);rows.append({"method":method,"metric":m,"participant_mean":b[m].mean(),"participant_median":b[m].median(),"bootstrap_low":lo,"bootstrap_high":hi,"participants":b.participant.nunique()})
        return rows,per
