...
def natural_metrics_v28(y:np.ndarray,clean:np.ndarray,eog:np.ndarray,c_query:np.ndarray,eeg_scale:np.ndarray)->dict[str,float|str]:
    physical_y=y*eeg_scale[:,None];physical_clean=clean*eeg_scale[:,None];correction=physical_y-physical_clean;e=eog-np.mean(eog,axis=1,keepdims=True);field=c_query@e;energy=np.sqrt(np.mean(e*e,axis=0));lo=energy<=np.quantile(energy,.3);hi=energy>=np.quantile(energy,.7)
