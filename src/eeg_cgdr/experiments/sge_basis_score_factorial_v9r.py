...
def stage_v9_audit(config: Mapping[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    for key in sorted({row["recording_key"] for row in rows}):
        ...
        eligible_field = str(cal[(key, "D11")].get("personalization_eligible", "")).strip()
        eligible = 0 if eligible_field == "0" or cal[(key,"D11")].get("fallback_reason","").strip() else 1
...
def stage_technical(config:Mapping[str,Any],index:int,run_dir:Path)->dict[str,Any]:
    fold_ids=list(config["technical_folds"]);fold_id=fold_ids[index];fold=next(r for r in _folds(config) if r["fold_id"]==fold_id);checkpoint=_old_checkpoint(config,fold_id);arrays=_old_arrays(config,fold_id);pop=np.asarray(checkpoint["population_basis"],np.float32);tau=np.asarray(checkpoint["tau"],np.float32);key=fold["heldout"][0];match,mask,_,_=_geometry(config,fold,arrays,pop,key,int(config["pair_seed"]));builder="v9_exact"
    try:adapt=_support_pairs(config,fold,arrays,key,pop,tau,split="adapt",shuffled=False,seed=int(config["pair_seed"]));valid=_support_pairs(config,fold,arrays,key,pop,tau,split="validation",shuffled=False,seed=int(config["pair_seed"]))
    except ValueError:
        layouts,records=_metadata(config);loaded=load_sgeyesub_signal_record(Path(str(config["data_root"])),records[key],layouts[records[key].layout_id],include_query=False,include_query_annotations=False);rate=float(fold["sampling_rate_hz"]);taps=2*int(round(float(config["fir_lag_ms"])*rate/1000))+1
...
def stage_exact_aggregate(config:Mapping[str,Any],run_dir:Path)->dict[str,Any]:
    ...
    _csv(root/"adapter_seed_variance.csv",variance);boot=[]
    for panel,values in (("raw_itt",unit),("raw_eligible",eligible)):
        boot.extend({"panel":panel,**row} for row in _cluster_bootstrap(values,("U_D","U_P","U_W","U_S","I","D00_DET_minus_DIFF"),int(config["bootstrap_seed"]),int(config["bootstrap_replicates"])))
    ...
    _csv(root/"natural_safety_distribution.csv",safety);safety_summary={name:{"mean":float(np.mean([r[name] for r in safety])),"median":float(np.median([r[name] for r in safety])),"minimum":float(np.min([r[name] for r in safety]))} for name in ("eog_coherence_reduction","nonartifact_preservation","psd_distortion","covariance_distortion")}
    severe_flags=[r["nonartifact_preservation"]<.70 or r["psd_distortion"]>.35 or r["covariance_distortion"]>.35 for r in safety]
    severe=float(np.mean(severe_flags));active=sum(r["adapter_active"] for r in eligible)/max(len(eligible),1);means={e:float(np.mean([r[e] for r in eligible])) for e in ("U_D","U_P","U_W","U_S","I")};gate=means["U_D"]>0 and means["I"]>0 and means["U_W"]>0 and means["U_S"]>0 and means["U_P"]>-.01 and active>=.8 and severe<=.25 and safety_summary["nonartifact_preservation"]["mean"]>=.70 and safety_summary["psd_distortion"]["mean"]<=.35 and safety_summary["covariance_distortion"]["mean"]<=.35
    if not gate:
        if active<.8:decision="SUPPORT_PROTOCOL_APPLICABILITY_LIMITED"
        elif means["I"]<=0:decision="V9_INTERACTION_NOT_REPRODUCED_UNDER_PAIRED_ADAPTATION_RANDOMNESS"
        else:decision="V9_D11_INTERACTION_PRESENT_BUT_SUBJECT_UTILITY_HETEROGENEOUS"
...
def stage_new_fold_aggregate(config:Mapping[str,Any],run_dir:Path)->dict[str,Any]:
    ...
    summary={"status":"completed_scientific_pass" if gate else "completed_scientific_no_go","decision":decision,"full_one_seed_expansion_eligible":gate,"means_itt":means_itt,"means_eligible":means,"equal_study_U_P":equal_study,"eligible_study_nonnegative_U_P":study_nonnegative,"eligible_studies":len(eligible_studies),"U_P_win_fraction":win,"personalization_coverage":coverage,"eligible_units":len(eligible),"itt_units":len(units),"safety":smean,"confirmation":False};_json(root/"routing_decision.json",summary);_json(run_dir/"result_summary.json",summary);return summary


def stage_finalize(config:Mapping[str,Any],run_dir:Path)->dict[str,Any]:
    root=_result(config);decision=json.loads((root/"exact_route_decision.json").read_text());coverage=json.loads((root/"support_coverage_summary.json").read_text());summary_rows=_read(root/"effect_summary.csv");eligible={r["effect"]:r for r in summary_rows if r["panel"]=="raw_eligible"};itt={r["effect"]:r for r in summary_rows if r["panel"]=="raw_itt"};boot={(r["panel"],r["effect"]):r for r in _read(root/"bootstrap_summary.csv")};new_path=_new_fold_root(config)/"routing_decision.json";new=json.loads(new_path.read_text()) if new_path.exists() else {"decision":"NOT_RUN","full_one_seed_expansion_eligible":False};lines=["# SGE basis × Score-LoRA V9R","","Development exploration; no confirmation outcomes were opened. Support pseudo-pairs use support artifact-class labels, so the method is **label-assisted calibration support**, not unsupervised calibration.","","## Exact common-random replay","","| effect | ITT mean | eligible mean | equal-study | median | positive | eligible descriptive 95% CI |","|---|---:|---:|---:|---:|---:|---:|"]
    for effect in ("D00_DET_minus_DIFF","U_D","U_P","U_W","U_S","I"):
        r=eligible[effect];q=itt[effect];b=boot[("raw_eligible",effect)];lines.append(f"| {effect} | {float(q['stem_weighted_mean']):+.4f} | {float(r['stem_weighted_mean']):+.4f} | {float(r['equal_study_mean']):+.4f} | {float(r['median']):+.4f} | {r['positive_count']}/{r['units']} | [{float(b['ci_low']):+.4f}, {float(b['ci_high']):+.4f}] |")
    safety=decision["safety"];lines += ["",f"Population diffusion D00 is a heterogeneous positive-average development signal: eligible DET−DIFF {float(eligible['D00_DET_minus_DIFF']['stem_weighted_mean']):+.4f}; its descriptive interval crosses zero.",f"Exact routing: `{decision['decision']}`. Interaction remained positive under paired randomness, but U_P was positive in only {eligible['U_P']['positive_count']}/{eligible['U_P']['units']} eligible stems and its equal-study mean was {float(eligible['U_P']['equal_study_mean']):+.4f}; this is not an established subject-utility claim.","","## Applicability and safety","",f"Original builder eligible: {coverage['old_eligible']}/{coverage['compatible_stems']}; unified blocked builder eligible: {coverage['new_eligible']}/{coverage['compatible_stems']} (availability denominator 59). Exact replay retained 14 ITT stems: 11 eligible and 3 POP fallbacks.",f"LoRA-active coverage among eligible stems: {decision['lora_active_coverage']:.1%}; severe natural-safety reversal fraction: {decision['severe_reversal_fraction']:.1%}.",f"Natural D11 means: EOG attenuation {safety['eog_coherence_reduction']['mean']:.4f}, preservation {safety['nonartifact_preservation']['mean']:.4f}, PSD distortion {safety['psd_distortion']['mean']:.4f}, covariance distortion {safety['covariance_distortion']['mean']:.4f}. Preservation minimum was {safety['nonartifact_preservation']['minimum']:.4f}.","","## Limited new-fold extrapolation","",f"The fold-ID-frozen extrapolation decision is `{new['decision']}`. Qualification for a later full one-seed expansion: `{new.get('full_one_seed_expansion_eligible',False)}`."]
    if new.get("means_itt"):
        lines += ["","| effect | ITT mean | eligible mean |","|---|---:|---:|"]
        for effect in ("U_D","U_P","U_W","U_S"):
            lines.append(f"| {effect} | {float(new['means_itt'][effect]):+.4f} | {float(new['means_eligible'][effect]):+.4f} |")
        ns=new["safety"]
        lines += ["",f"Frozen new folds covered {new['itt_units']} ITT stems and {new['eligible_units']} eligible stems (personalization coverage {new['personalization_coverage']:.1%}). U_P was positive in {new['U_P_win_fraction']:.1%} of eligible stems and in {new['eligible_study_nonnegative_U_P']}/{new['eligible_studies']} eligible studies; equal-study U_P was {new['equal_study_U_P']:+.4f}.",f"New-fold natural means: EOG attenuation {ns['eog_coherence_reduction']:.4f}, preservation {ns['nonartifact_preservation']:.4f}, PSD distortion {ns['psd_distortion']:.4f}, covariance distortion {ns['covariance_distortion']:.4f}.","", "The common-random interaction signal from the selected folds did not translate into positive subject utility on the fold-ID-frozen extrapolation. This is `V9_D11_INTERACTION_PRESENT_BUT_SUBJECT_UTILITY_HETEROGENEOUS`, not a family-wide negative result."]
    lines += ["","Raw is primary. V9 historical calibrated panels were corrected and retained separately; V9R did not reuse the historically selected per-subject correction calibration. All intervals are development-descriptive. No result is generalized to diffusion, Score-LoRA, or personalization families."]
    Path("reports/sge_basis_score_factorial_v9r.md").write_text("\n".join(lines)+"\n",encoding="utf-8")
    Path("reports/v9_common_randomness_audit.md").write_text("# V9 common-randomness audit\n\nAdaptationReplay freezes zero-output LoRA initialization, minibatches, diffusion timesteps, Gaussian noise, dropout RNG, checkpoint steps, and K=8 inference noise. D01/D11 replay the same schedule; donor adapters are trained once per fold/seed and reused. Rank masks are applied to targets, loss, and correction. Query inference sees deployable EEG/condition IDs only.\n\nThe first replay (job 928921) was invalidated before evaluation because repeated GPU adaptation and checkpoint reload were not bit-reproducible. Jobs 928959/928962/928964 localized cuDNN/CuBLAS and train/eval-mode causes. Job 928968 passed all real-record replay checks. The scientific replay 928971 used deterministic CUDA kernels and the revisioned `exact_replay_r2_deterministic_cuda` result root; invalid outputs remain separate.\n",encoding="utf-8")
    v9={r["effect"]:r for r in _read(root/"v9_audit"/"summary.csv") if r["panel"]=="raw_eligible"};correction=["# V9 statistical correction","","Historical V9 files were not modified. Raw and calibrated WRONG/SHUFFLED names are matched exactly, fallback zeros stay in ITT but not eligible-only effects, and eligibility is explicit 0/1.","","| effect | corrected V9 eligible mean | V9R paired-replay eligible mean |","|---|---:|---:|"]
    for effect in ("U_D","U_P","U_W","U_S","I"):correction.append(f"| {effect} | {float(v9[effect]['stem_weighted_mean']):+.4f} | {float(eligible[effect]['stem_weighted_mean']):+.4f} |")
    correction += ["","D00 DET−DIFF remains a heterogeneous positive-average development signal because its descriptive interval spans zero. The positive interaction reproduced, while subject utility remained heterogeneous across studies and stems."]
    Path("reports/v9_statistical_correction.md").write_text("\n".join(correction)+"\n",encoding="utf-8")
    final={"status":"completed_v9r","exact_replay":decision,"coverage":coverage,"new_fold_extrapolation":new,"confirmation":False,"family_wide_status":"not_tested"};_json(root/"result_summary.json",final);_json(run_dir/"result_summary.json",final);return final
