...
def aggregate(config:Mapping[str,Any],run_dir:Path,seeds:list[int])->Mapping[str,Any]:
    ...
    units=_mean_rows(successful,("participant","protocol","method"),metrics);output=root/"aggregate";_write(output/"unit_metrics.csv",units)
    ...
    for protocol in ("S0_STATIC_XSESSION","S1_MOTION_WITHIN_SESSION","S2_MOTION_XSPEED"):
        ...
        safety_mean=np.nanmean(np.asarray(safety),axis=0) if safety else np.full(5,np.nan);safety_pass=bool(safety and np.nanmin(safety_mean)>=margin)
    ...
    expand=any(value["additional_seeds_authorized"] for value in decisions.values());summary={"status":"completed_v5_one_seed_factorial_aggregation" if len(seeds)==1 else "completed_v5_three_seed_stability","seeds":seeds,"protocol_decisions":decisions,"additional_seeds_authorized":expand,"score_adapter_backup_authorized":backup,"population_diffusion_signal":population_diffusion_signal,"context_collapse":context_collapse,"sealed_signal_opened":False,"confirmation_eligibility":False,"family_wide_status":"not_tested"};(output/"routing_decision.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n");run_dir.mkdir(parents=True,exist_ok=True);(run_dir/"result_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n");return summary
