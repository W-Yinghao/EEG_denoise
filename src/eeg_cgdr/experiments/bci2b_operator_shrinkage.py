...
def stage_aggregate(c:Mapping[str,Any],task_index:int,run:Path)->dict[str,Any]:
    ...
    _csv(root/"participant_seed_effects.csv",seed_effects);participant_effect_rows=[{"participant":p,**{key:float(np.mean([r[key] for r in seed_effects if r["participant"]==p])) for key in ("E_P","E_W","E_D","Delta_SA")}} for p in range(1,10)];_csv(root/"participant_effects.csv",participant_effect_rows)
...
def stage_oppost_infer(c:Mapping[str,Any],task_index:int,run:Path)->dict[str,Any]:
    ...
    for unit_index,protocol in enumerate(SAME):
        ...
        hp=np.asarray(inf["h_pop"]);hm_likelihood=_half_support_transfer(c,recipient,manifest["support_session"],inf);hm_current=np.asarray(inf["h_match"]);donors=[int(x) for x in manifest["wrong_donors"].split(";") if x];outputs={}
        for panel in ("paired","natural"):
            ...
            with torch.no_grad():rd=det(y=yt,eog=et,a0=at)
            contexts=[("POP",hp,recipient),("MATCH",hm_likelihood,recipient)]+[(f"WRONG-{d}",_half_support_transfer(c,d,manifest["support_session"],inf),d) for d in donors]
            ...
            am=v11.apply_transfer(hm_current,eog);amt=torch.as_tensor(am,device=device)
...
def stage_oppost_aggregate(c:Mapping[str,Any],task_index:int,run:Path)->dict[str,Any]:
    ...
    selected_by_participant={int(r["fold"])+1:r for r in selected};selected_weights=[]
    for row in oracle:
        chosen=selected_by_participant.get(int(row["participant"]))
        if chosen and row["center"]=="support" and int(float(row["late_steps"]))==int(chosen["late_steps"]) and float(row["w_max"])==float(chosen["w_max"]):selected_weights.append(row)
    weight_fields=("weight_min","weight_median","weight_max","fraction_steps_gt_0_10","fraction_steps_gt_0_25","fraction_steps_gt_0_50","fraction_steps_gt_0_90")
    weight_summary={key:float(np.mean([float(r[key]) for r in selected_weights])) for key in weight_fields};schedule_counts=defaultdict(int)
    for row in selected:schedule_counts[f"late={int(row['late_steps'])},wmax={float(row['w_max']):g}"]+=1
    oracle_gain=[]
    for participant in range(1,10):
        for protocol in SAME:
    ...
    summary={"effects":summary_effect,"natural":nat,"current_natural":cur,"natural_margins":margin,"selected_weight_summary":weight_summary,"frozen_schedule_counts":dict(schedule_counts),"oracle_mean_gain":float(np.mean(oracle_gain)) if oracle_gain else None,"oracle_positive":int(sum(x>0 for x in oracle_gain)),"oracle_units":len(oracle_gain),"oracle_mean_rrmse_by_w_max":oracle_means,"routing":route}
    _json(root/"result_summary.json",summary);_json(root/"routing_decision.json",route)
    figdir=root/"figures";figdir.mkdir(parents=True,exist_ok=True);fig,axes=plt.subplots(1,2,figsize=(9,3.6));axes[0].axhline(0,color="black",lw=.8);axes[0].plot(range(1,10),summary_effect["U_P"]["participant_values"],"o-",label="U_P");axes[0].plot(range(1,10),summary_effect["U_W"]["participant_values"],"s-",label="U_W");axes[0].set_xlabel("Participant");axes[0].set_ylabel("Positive RRMSE utility");axes[0].legend();points=sorted((float(k),v) for k,v in oracle_means.items());axes[1].plot([a for a,_ in points],[b for _,b in points],"o-");axes[1].set_xlabel("w_max");axes[1].set_ylabel("Oracle artifact RRMSE");fig.tight_layout();fig.savefig(figdir/"bounded_oppost_diagnostic.png",dpi=180);plt.close(fig)
    Path("reports/bci2b_oppost_strength_audit.md").write_text(f"# BCI2b bounded OPPOST strength audit\n\nDecision: `{status}`. One-seed participant-first U_P={summary_effect['U_P']['mean']:+.5f} ({summary_effect['U_P']['positive']}/9), U_W={summary_effect['U_W']['mean']:+.5f} ({summary_effect['U_W']['positive']}/9), and G_MATCH={summary_effect['G_MATCH']['mean']:+.5f}. Oracle-center mean artifact-RRMSE gain is {summary['oracle_mean_gain']} ({summary['oracle_positive']}/{summary['oracle_units']}); monotonicity across the frozen w_max schedule is {oracle_monotonic}. Selected-schedule mean weight min/median/max is {weight_summary['weight_min']:.4f}/{weight_summary['weight_median']:.4f}/{weight_summary['weight_max']:.4f}; mean active-step fractions above 0.10/0.25/0.50/0.90 are {weight_summary['fraction_steps_gt_0_10']:.3f}/{weight_summary['fraction_steps_gt_0_25']:.3f}/{weight_summary['fraction_steps_gt_0_50']:.3f}/{weight_summary['fraction_steps_gt_0_90']:.3f}. Schedule selection used outer-training rows only and was frozen before held-out outputs. Natural margins versus CURRENT-DIFF-MATCH were preservation {margin['preservation']:+.4f}, MI-kappa {margin['mi_kappa']:+.4f}, PSD {margin['mi_band_distortion']:+.4f}, and covariance {margin['covariance']:+.4f}. The valid oracle center but failed support center establishes an operator-likelihood bridge failure; no extra seeds or localized backup were run.\n",encoding="utf-8")
...
def stage_bci2a_aggregate(c:Mapping[str,Any],task_index:int,run:Path)->dict[str,Any]:
    ...
    for participant in range(1,10):
        ...
        wrong=[np.mean(v) for k,v in by.items() if k.startswith("SHRINK-WRONG-")];pop=float(np.mean(by["POP8"]));match=float(np.mean(by["SHRINK-MATCH"]));wrong_mean=float(np.mean(wrong));effects.append({"participant":participant,"E_P":pop-match,"E_W":wrong_mean-match,"E_P_relative":(pop-match)/(pop+1e-12),"E_W_relative":(wrong_mean-match)/(wrong_mean+1e-12),"preservation_margin":float(np.mean([float(r["preservation"]) for r in take if r["method"]=="SHRINK-MATCH"])-np.mean([float(r["preservation"]) for r in take if r["method"]=="POP8"]))})
    _csv(root/"participant_effects.csv",effects);ep=np.asarray([r["E_P"] for r in effects]);ew=np.asarray([r["E_W"] for r in effects]);epr=np.asarray([r["E_P_relative"] for r in effects]);ewr=np.asarray([r["E_W_relative"] for r in effects]);pm=np.asarray([r["preservation_margin"] for r in effects]);passed=bool(len(effects)==9 and ep.mean()>0 and np.median(ep)>0 and (ep>0).sum()>=6 and ew.mean()>0 and np.median(ew)>0 and pm.mean()>=-.02);route={"status":"BCI2A_OPERATOR_SHRINKAGE_HEADROOM_DETECTED" if passed else "BCI2A_OPERATOR_SHRINKAGE_HEADROOM_NOT_ESTABLISHED","one_seed_factorial_authorized":passed,"eog_transfer_personalization_family_stopped":not passed,"development_only":True};summary={"effects":{"E_P_mean":float(ep.mean()),"E_P_median":float(np.median(ep)),"E_P_relative_mean":float(epr.mean()),"E_P_relative_median":float(np.median(epr)),"E_P_positive":int((ep>0).sum()),"E_W_mean":float(ew.mean()),"E_W_median":float(np.median(ew)),"E_W_relative_mean":float(ewr.mean()),"E_W_relative_median":float(np.median(ewr)),"E_W_positive":int((ew>0).sum()),"preservation_margin_mean":float(pm.mean())},"routing":route};_json(root/"result_summary.json",summary);_json(root/"routing_decision.json",route);Path("reports/bci2a_operator_shrinkage_headroom.md").write_text(f"# BCI2a operator shrinkage headroom\n\nConditional development audit decision: `{route['status']}`. Relative E_P mean/median {epr.mean():+.5f}/{np.median(epr):+.5f} with {(ep>0).sum()}/9 positive; relative E_W mean/median {ewr.mean():+.5f}/{np.median(ewr):+.5f}. The same 120-second support-only lambda rule was used; later EEG/EOG was opened only by this evaluator.\n",encoding="utf-8");_json(run/"result_summary.json",summary);return summary
