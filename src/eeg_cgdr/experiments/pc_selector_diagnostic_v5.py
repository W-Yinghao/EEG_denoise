...
def run(config:Mapping[str,Any],run_dir:Path)->Mapping[str,Any]:
    ...
    for dataset in ("klados","sgeyesub"):
        for route in ("M0","M1"):
            selected=[row for row in rows if row["dataset"]==dataset and row["route"]==route and str(row["status"]).startswith("success")];summaries.append({"dataset":dataset,"route":route,"denominator":16 if dataset=="klados" else 59,"successful":len(selected),"mean_label_prevalence":float(np.mean([row["label_prevalence"] for row in selected])) if selected else float("nan"),"mean_auroc":float(np.nanmean([row["auroc"] for row in selected])) if selected else float("nan"),"mean_auprc":float(np.nanmean([row["auprc"] for row in selected])) if selected else float("nan"),"mean_coverage":float(np.mean([row["coverage"] for row in selected])) if selected else 0.,"matching_selected_safe_rate":float(np.mean([row["selected_safe_benefit_rate"] for row in selected])) if selected else 0.,"wrong_support_selected_safe_rate":float(np.mean([row["wrong_support_selected_safe_benefit_rate"] for row in selected])) if selected else 0.})
