...
def stage_final(c: Mapping[str, Any], task_index: int, run: Path) -> dict[str, Any]:
    ...
    historical_path = _root(c, "previous_root") / "capacity_matched" / "paired_metrics.csv"
    historical = None
    if historical_path.exists():
        rows = [row for row in _read(historical_path) if row["method"] == "DIFF25-K8-POP8"]
        participant_values = [
            {"participant": participant, "rrmse": float(np.mean([float(row["rrmse"]) for row in rows if int(row["participant"]) == participant]))}
            for participant in range(1, 10)
        ]
        historical = {
            "method": "DIFF25-K8-POP8",
            "role": "historical_EOG_guided_POP8_R_reference_only",
            "source": str(historical_path),
            "rrmse_participant_first": float(np.mean([row["rrmse"] for row in participant_values])),
            "participants": 9,
            "seeds": sorted({int(row["seed"]) for row in rows}),
            "not_information_matched_to_raw_support_model": True,
        }
        _csv(_root(c, "result_root") / "historical_pop8_reference.csv", participant_values)
    route = {"eog_transfer_personalization_family": "closed", "covariance_actionability": oracle["routing"]["status"], "raw_temporal_support_clean_diffusion": model["routing"]["status"] if model else "NOT_RUN_TECHNICAL_FAILURE", "additional_training_seeds_run": bool(model and len(model["seeds"]) > 1), "development_only": True, "family_wide_negative_forbidden": True}; summary = {"corrected_geometry": geometry, "oracle_actionability": oracle, "raw_support_clean_diffusion": model, "historical_pop8_reference": historical, "routing": route}; _json(_root(c, "result_root") / "result_summary.json", summary); _json(_root(c, "result_root") / "route_decision.json", route)
    lines = ["# Corrected neural geometry and oracle actionability", "", "The historical covariance label is corrected to **robust spatial-correlation geometry** because each channel is median/MAD standardized. Support and later-query evaluator windows both use the frozen lowest-30% EOG-energy rule. AIRM populations use a true affine-invariant Karcher barycenter; log-Euclidean populations use a log-domain barycenter. Historical outputs remain unchanged.", "", "The scientific unit is the participant (n=9), after session aggregation; support and later-query raw time blocks are disjoint.", ""]
    for metric in ("H_P_airm", "H_W_airm", "H_P_logeuclidean", "H_W_logeuclidean"):
        value = geometry["metrics"][metric]
        lines.append(f"- {metric}: mean {value['mean']:+.6f}, median {value['median']:+.6f}, {value['positive']}/9, one-sided exact p={value['one_sided_exact_sign_flip']:.6f}.")
    oracle_uo = oracle["effects"]["U_O_DIFF25-K8"]
    oracle_uow = oracle["effects"]["U_OW_DIFF25-K8"]
    lines.extend(["", f"Oracle route: `{oracle['routing']['status']}`. The query-clean covariance context is evaluator-only and is not deployable.", f"- U_O: mean {oracle_uo['mean']:+.6f}, median {oracle_uo['median']:+.6f}, {oracle_uo['positive']}/9.", f"- U_OW: mean {oracle_uow['mean']:+.6f}, median {oracle_uow['median']:+.6f}, {oracle_uow['positive']}/9.", "The corrected geometry is subject-identifiable, but its oracle clean-target actionability gate fails; no whitening/alignment model was trained.", ""])
    Path("reports/corrected_neural_geometry_oracle.md").write_text("\n".join(lines), encoding="utf-8")
    if model:
        for name in ("participant_effects.csv", "method_summary_participant_first.csv", "participant_natural_safety.csv", "method_compute.csv"):
            _csv(_root(c, "result_root") / name, _read(_root(c, "model_root") / name))
        effects = model["effects"]; methods = {row["method"]: row for row in model["method_summary"]}; report = ["# Raw temporal support-conditioned clean EEG diffusion", "", f"Decision: `{model['routing']['status']}` on the full BCI2b same-session development cohort.", "", "The diffusion state is the full clean EEG waveform. Query inference sees corrupted EEG and sixteen raw temporal EEG support patches; it never sees query EOG, labels, participant ID, covariance/PSD summaries, or clean targets. Twenty-six of 27 protocol units are eligible; all nine participants remain the scientific denominator.", "", "## Technical validity", "", "The real-data technical gate passed: exact checkpoint/replay, finite gradients for all trainable tensors, support-set permutation invariance, nonzero MATCH/WRONG response, query-shuffle sensitivity, fixed-batch DET/DIFF reconstruction, and outer-training validation improvement over RAW. DET and DIFF each have 943,683 trainable parameters.", "", "## Participant-first effects", ""]
        report.extend(f"- {key}: mean {value['mean']:+.5f}, median {value['median']:+.5f}, {value['positive']}/9, one-sided exact p={value['one_sided_exact_sign_flip']:.6f}, descriptive 95% interval [{value['descriptive_ci'][0]:+.5f}, {value['descriptive_ci'][1]:+.5f}]." for key, value in effects.items())
        report.extend(["", "U_P fails the frozen subject-utility gate (mean below +0.005 and only 5/9 positive); U_W is positive but cannot rescue U_P. Accordingly, no extra training seeds were submitted. E_D and E_K are positive secondary signals for this one seed, not evidence that subject conditioning succeeded.", "", "## Absolute performance and safety", "", f"- RAW paired RRMSE: {methods['RAW']['rrmse']:.5f}.", f"- DET-CLEAN-MATCH paired RRMSE: {methods['DET-CLEAN-MATCH']['rrmse']:.5f}.", f"- DIFF-CLEAN-POP-K8 paired RRMSE: {methods['DIFF-CLEAN-POP-K8']['rrmse']:.5f}.", f"- DIFF-CLEAN-MATCH-K8 paired RRMSE: {methods['DIFF-CLEAN-MATCH-K8']['rrmse']:.5f}.", f"- Natural MATCH means: EOG attenuation {model['natural_match']['eog_attenuation']:+.5f}, preservation {model['natural_match']['preservation']:.5f}, covariance {model['natural_match']['covariance']:.5f}, MI-band distortion {model['natural_match']['mi_band_distortion']:.5f}, MI kappa {model['natural_match']['mi_kappa']:.5f}, ERD preservation {model['natural_match']['erd_preservation']:.5f}.", f"- MATCH-minus-POP safety margins: EOG attenuation {model['natural_margins']['eog_attenuation']:+.5f}, preservation {model['natural_margins']['preservation']:+.5f}, covariance {model['natural_margins']['covariance']:+.5f}, MI-band distortion {model['natural_margins']['mi_band_distortion']:+.5f}, MI kappa {model['natural_margins']['mi_kappa']:+.5f}, ERD {model['natural_margins']['erd_preservation']:+.5f}."])
        if historical:
            report.extend(["", "## Historical reference", "", f"The frozen EOG-guided DIFF25-K8-POP8-R result has participant-first RRMSE {historical['rrmse_participant_first']:.5f} over three seeds. It is reported only as a historical reference because it uses query EOG and a different estimator, and is not an information-matched arm of this EEG-only raw-support model."])
        report.extend(["", "## Boundary", "", "This is one-seed development evidence. The frozen temporal Conv/set-token/cross-attention clean-conditional instance did not establish unseen-subject utility and is closed under the preregistered stopping rule. This does not imply a diffusion- or personalization-family-wide negative; no additional architectures, patch/token grids, or sampler variants were run."])
