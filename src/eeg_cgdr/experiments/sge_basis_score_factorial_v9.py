...
def stage_factorial_infer(config:Mapping[str,Any],index:int,run_dir:Path)->dict[str,Any]:
    ...
    for position,key in enumerate(fold["heldout"]):
        ...
        if paired_path.exists() and natural_path.exists() and adaptation_path.exists():
            ...
            for candidate,values in adaptation.get("calibration",{}).items():calibration_rows.append({"fold_id":fold_id,"recording_key":key,"candidate":candidate,"gamma":values["gamma"],"w":values["w"],"support_validation_rrmse":adaptation.get("support_validation_rrmse",{}).get(candidate,float("nan"))})
            continue
...
def stage_aggregate(config:Mapping[str,Any],run_dir:Path)->dict[str,Any]:
    ...
    for candidate in ("D10","D01","D11"):
        ...
        cal_rows=[r for r in calibration if r["candidate"]==candidate];mean_w=float(np.mean([float(r["w"]) for r in cal_rows]));active=float(np.mean([float(r["w"])>=.25 for r in cal_rows]));method=f"DIFF-{candidate}-CAL";nrows=[r for r in natural if r["method"]==method];pres=float(np.nanmean([float(r["nonartifact_preservation"]) for r in nrows]));psd=float(np.nanmean([float(r["psd_distortion"]) for r in nrows]));cov=float(np.nanmean([float(r["covariance_distortion"]) for r in nrows]));severe=float(np.mean([r[f"{candidate}_CAL_U_P"]<-.05 for r in effects]));coverage=len(cal_rows)/max(len(effects),1);qualifies=coverage>=.8 and metrics["U_D"]>0 and np.mean(subject_values)>0 and sum(v>0 for v in subject_values)>=2 and min(subject_values)>=-.01 and subject_nonnegative_studies>=3 and severe<=.2 and pres>=.70 and psd<=.35 and cov<=.35;routes.append({"route":candidate,"coverage":coverage,**metrics,"mean_subject_effect":float(np.mean(subject_values)),"nonnegative_study_counts":json.dumps(study_counts,sort_keys=True),"subject_nonnegative_studies":subject_nonnegative_studies,"severe_reversal_fraction":severe,"preservation":pres,"psd_distortion":psd,"covariance_distortion":cov,"mean_w":mean_w,"w_ge_0_25_fraction":active,"diagnostic_qualifies":qualifies})
