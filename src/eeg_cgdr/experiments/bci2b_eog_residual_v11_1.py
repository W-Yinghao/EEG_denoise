...
def stage_finalize(config:Mapping[str,Any],task_index:int,run_dir:Path)->dict[str,Any]:
    ...
    secondary=result["summaries"]["32"]["effects"];route=result["routing_by_k"][str(k)]
    lines += ["",f"K32 was evaluated only as the frozen secondary panel. It produced U_D mean/median `{secondary['U_D']['mean']:+.4f}/{secondary['U_D']['median']:+.4f}` ({secondary['U_D']['positive']}/9), but U_L remained `{secondary['U_L']['mean']:+.4f}/{secondary['U_L']['median']:+.4f}` ({secondary['U_L']['positive']}/9). It did not replace K8 or establish diffusion increment.","",f"Population denoising passed: `{route['population_valid']}`. Subject operator utility passed: `{route['subject_operator_supported']}`. Diffusion increment passed: `{route['diffusion_increment_supported']}`. No additional seeds were authorized."]
    if (root/"inference_resource_summary.csv").exists():
        lines += ["","| panel | latency/window | posterior calls | peak GPU memory |","|---|---:|---:|---:|"]
        for row in _read(root/"inference_resource_summary.csv"):lines.append(f"| K{row['k']}/DDIM25 | {float(row['latency_seconds_per_window']):.4f} s | {row['posterior_network_calls']} | {int(row['peak_memory_bytes_max'])/1e6:.0f} MB |")
