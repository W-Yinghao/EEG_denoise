...
def run(config:Mapping[str,Any],run_dir:Path)->Mapping[str,Any]:
    root=CODE_ROOT/str(config["output_root"]);headroom=json.loads((root/"aggregate/routing_decision.json").read_text());evaluator=json.loads((root/"evaluator/result_summary.json").read_text());pc=json.loads((CODE_ROOT/str(config["pc_output_root"])/"result_summary.json").read_text()) if (CODE_ROOT/str(config["pc_output_root"])/"result_summary.json").is_file() else {"decision":"INDETERMINATE_CONSTRAINT_FEASIBILITY"};effects,diffusion=_diffusion_summary(config);final=root/"final";final.mkdir(parents=True,exist_ok=True)
    if effects:_write(final/"temporal_diffusion_paired_effects.csv",effects)
    split=json.loads((root/"metadata/split_balance.json").read_text());summary={"status":"completed_mobile_headroom_temporal_v4","v3_evidence_corrected":True,"pc_constrained_oracle":pc.get("decision"),"mobile_science_ready":evaluator.get("science_ready",False),"headroom_routing":headroom.get("routing_decision"),"protocol_headroom":headroom.get("protocol_decisions"),"temporal_diffusion":diffusion,"sealed_participants":split["sealed"],"sealed_signal_marker_outcome_opened":False,"confirmation_eligibility":False,"family_wide_status":"not_tested_no_family_wide_claim_allowed"};(final/"result_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    ...
    units=_read(root/"aggregate/unit_metrics.csv");fig,axis=plt.subplots(figsize=(5,4));colors={"STRONG-POP":"tab:blue","DET-MATCH":"tab:orange","DET-NULL":"tab:green"}
    ...
    main=CODE_ROOT/"reports/mobile_bci_headroom_v4.md";main.write_text(main.read_text()+"\n## Final v4 state\n\n"+f"P-C: `{pc.get('decision')}`. Mobile evaluator science-ready: `{evaluator.get('science_ready')}`. Headroom: `{headroom.get('routing_decision')}`. Temporal diffusion: `{diffusion.get('status')}`. No result is confirmation or a family-wide verdict.\n",encoding="utf-8")
