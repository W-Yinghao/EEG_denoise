...
def fir_coefficients_from_lag_major(
    ...
    *,
    ...
    """Convert ridge coefficients from ``[lag, EOG]`` to ``(C,E,L)``.

    ``_fir_design`` concatenates one complete EOG block per lag. A direct
    ``reshape(C,E,L)`` therefore silently swaps axes when E and L exceed one.
    """

    value = np.asarray(coefficients, dtype=np.float64)
    expected = (int(eeg_channels), int(eog_channels) * int(lag_count))
    if value.shape != expected:
        raise ValueError(f"lag-major FIR coefficient shape must be {expected}")
    return value.reshape(eeg_channels, lag_count, eog_channels).transpose(0, 2, 1)


def fir_coefficients_to_lag_major(transfer: np.ndarray) -> np.ndarray:
    """Inverse of :func:`fir_coefficients_from_lag_major`."""

    value = np.asarray(transfer, dtype=np.float64)
    if value.ndim != 3:
        raise ValueError("FIR transfer must have shape (C,E,L)")
    return value.transpose(0, 2, 1).reshape(value.shape[0], -1)


def interpolated_fir_transfer(
    population_transfer: Tensor,
    subject_transfer: Tensor,
    rho: float | Tensor,
    *,
    ...
    """Return ``H0 + rho * (Hs-H0)`` with an explicit batch dimension."""

    population = torch.as_tensor(population_transfer)
    subject = torch.as_tensor(subject_transfer, device=population.device, dtype=population.dtype)
    if population.ndim == 3:
        population = population[None].expand(batch, -1, -1, -1)
    if subject.ndim == 3:
        subject = subject[None].expand(batch, -1, -1, -1)
    if population.shape != subject.shape or population.shape[0] != batch:
        raise ValueError("population/subject FIR transfers differ")
    reliability = torch.as_tensor(rho, device=population.device, dtype=population.dtype)
    if reliability.ndim == 0:
        reliability = reliability.expand(batch)
    if reliability.shape != (batch,) or bool(((reliability < 0) | (reliability > 1)).any()):
        raise ValueError("FIR rho must have shape (B,) and lie in [0,1]")
    return population + reliability[:, None, None, None] * (subject - population)


def fir_full_replacement(
    observed: Tensor,
    canonical_latent: Tensor,
    population_transfer: Tensor,
    subject_transfer: Tensor,
    ...
    """Apply the single repaired R2 equation ``x=y-H_rho*a``."""

    effective = interpolated_fir_transfer(
        population_transfer,
        subject_transfer,
        rho,
        batch=observed.shape[0],
    ).to(device=observed.device, dtype=observed.dtype)
    correction = fir_transfer_correction(effective, canonical_latent, lags, valid_time_mask)
    mask = torch.as_tensor(valid_time_mask, device=observed.device).bool()[:, None]
    restored = torch.where(mask, observed - correction, torch.zeros_like(observed))
    return restored, correction, effective
...
__all__ = [
    ...
    "canonical_eog_latent", "fir_coefficients_from_lag_major",
    "fir_coefficients_to_lag_major", "fir_full_replacement",
    "fir_transfer_correction", "full_c_subject_residual", "interpolated_fir_transfer",
