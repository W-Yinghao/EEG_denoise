...
class ResBlock1D(nn.Module):
    """Residual block with time-embedding injection (handoff §4)."""
...
class UNet1D(nn.Module):
    """1D U-Net predicting the noise ``ε_θ(x_t, t)`` for a 22-channel EEG window."""
    ...
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        def block(in_ch: int, out_ch: int) -> nn.ModuleList:
            blocks: List[nn.Module] = [ResBlock1D(in_ch, out_ch, tdim, groups, drop)]
            for _ in range(cfg.num_res_blocks - 1):
                blocks.append(ResBlock1D(out_ch, out_ch, tdim, groups, drop))
    ...
    @staticmethod
    def _run(blocks: nn.ModuleList, h: Tensor, temb: Tensor) -> Tensor:
        for blk in blocks:
            h = blk(h, temb)
        return h
    ...
    def forward(self, x: Tensor, t: Tensor) -> Tensor:
        ...
        h = self.stem(x)
        s0 = self._run(self.enc0, h, temb)
        s1 = self._run(self.enc1, self.down0(s0), temb)
        s2 = self._run(self.enc2, self.down1(s1), temb)

        h = self.down2(s2)
        h = self.mid2(self.mid_attn(self.mid1(h, temb)), temb)

        h = self._run(self.dec2, torch.cat([self.up2(h), s2], dim=1), temb)
        h = self._run(self.dec1, torch.cat([self.up1(h), s1], dim=1), temb)
        h = self._run(self.dec0, torch.cat([self.up0(h), s0], dim=1), temb)
